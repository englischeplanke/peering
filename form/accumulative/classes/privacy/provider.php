...
/**
 * Provides the class {@link peeringform_accumulative\privacy\provider}
 *
...
namespace peeringform_accumulative\privacy;
...
class provider implements \core_privacy\local\metadata\null_provider, \mod_peering\privacy\peeringform_provider {
    ...
    public static function export_assessment_form(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        $sql = "SELECT dim.id, dim.description, dim.descriptionformat, dim.grade AS dimgrade, dim.weight,
                  ...
                  JOIN {peeringform_accumulative} dim ON dim.peeringid = w.id
        ...
        foreach ($DB->get_records_sql($sql, $params) as $record) {
            ...
            $record->description = $writer->rewrite_pluginfile_urls($subcontext, 'peeringform_accumulative',
        ...
        if ($hasdata) {
            ...
            foreach ($dimensionids as $dimensionid) {
                $writer->export_area_files($subcontext, 'peeringform_accumulative', 'description', $dimensionid);
            }
        }
    }
}
