 ...
 * @uses moodleform
 */
class peering_accumulative_assessment_form extends peering_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'peeringform_accumulative', $i+1);
            ...
            $label = get_string('dimensiongradefor', 'peeringform_accumulative', $dimtitle);
            if ($fields->{'grade__idx_' . $i}) {
                ...
                $mform->addRule(array('grade__idx_' . $i, 'minusone'),
                    get_string('mustchoosegrade', 'peeringform_accumulative'), 'compare', 'gt');
            }
            // comment
            $label = get_string('dimensioncommentfor', 'peeringform_accumulative', $dimtitle);
