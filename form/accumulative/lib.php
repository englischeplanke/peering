 ...
 * @return bool
 */
function peeringform_accumulative_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peeringform_accumulative', array('id' => $itemid ,'peeringid' => $peering->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peeringform_accumulative/$filearea/$itemid/$relativepath";
 ...
 */
class peering_accumulative_strategy implements peering_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'peeringform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_edit_accumulative_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->accumulative;  // records to be saved into {peeringform_accumulative}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('peeringform_accumulative', $record);
            } else {
                // exiting field
                $DB->update_record('peeringform_accumulative', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'peeringform_accumulative', 'description', $record->id);
            $DB->update_record('peeringform_accumulative', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'peeringform_accumulative', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_accumulative_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id, d.grade, d.weight, s.scale
                  FROM {peeringform_accumulative} d
    ...
    public static function scale_used($scaleid, $peeringid=null) {
        ...
        if (!is_null($peeringid)) {
            $conditions['peeringid'] = $peeringid;
        }
        return $DB->record_exists('peeringform_accumulative', $conditions);
    }
    ...
    public static function delete_instance($peeringid) {
        ...
        $DB->delete_records('peeringform_accumulative', array('peeringid' => $peeringid));
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'peeringform_accumulative', 'description', $id);
            }
        }
        $DB->delete_records_list('peeringform_accumulative', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peering_edit_accumulative_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->accumulative = array();        // records to be stored in {peeringform_accumulative}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->accumulative[$i]                     = new stdclass();
            $cook->accumulative[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->accumulative[$i]->peeringid         = $this->peering->id;
