 ...
 */
abstract class peering_rubric_assessment_form extends peering_assessment_form {
    ...
    public function validation($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            if (empty($data['chosenlevelid__idx_'.$i])) {
                $errors['chosenlevelid__idx_'.$i] = get_string('mustchooseone', 'peeringform_rubric'); // used in grid
                $errors['levelgrp__idx_'.$i] = get_string('mustchooseone', 'peeringform_rubric');      // used in list
            }
        }
        return $errors;
    }
 ...
 */
class peering_rubric_list_assessment_form extends peering_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $nodims; $i++) {
            // dimension header
            $dimtitle = get_string('dimensionnumber', 'peeringform_rubric', $i+1);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $levelgrp[] = $mform->createElement('radio', 'chosenlevelid__idx_'.$i, '',
                        format_text($definition, $definitionformat, null, $peering->course->id), $levelid);
            }
 ...
 */
class peering_rubric_grid_assessment_form extends peering_rubric_assessment_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $numofcolumns = array_reduce($levelcounts, 'peering::lcm', 1);

        $mform->addElement('header', 'rubric-grid-wrapper', get_string('layoutgrid', 'peeringform_rubric'));

        $mform->addElement('html', '<table class="rubric-grid">' . "\n");
        $mform->addElement('html', '<th class="header">' . get_string('criteria', 'peeringform_rubric') . '</th>');
        $mform->addElement('html', '<th class="header" colspan="'.$numofcolumns.'">'.get_string('levels', 'peeringform_rubric').'</th>');
