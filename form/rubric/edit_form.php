 ...
 * @uses moodleform
 */
class peering_edit_rubric_strategy_form extends peering_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'peeringform_rubric', $i+1));
            ...
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                                get_string('dimensiondescription', 'peeringform_rubric'), '', $descriptionopts);
            ...
            for ($j = 0; $j < $numoflevels; $j++) {
                ...
                $mform->addGroup($levelgrp, 'level__idx_'.$i.'__idy_'.$j, get_string('levelgroup', 'peeringform_rubric'), array(' '), false);
        ...
        $mform->addElement('submit', 'adddims', get_string('addmoredimensions', 'peeringform_rubric',
                peering_rubric_strategy::ADDDIMS));
        $mform->closeHeaderBefore('adddims');

        $mform->addElement('header', 'configheader', get_string('configuration', 'peeringform_rubric'));
        $layoutgrp = array();
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutlist', 'peeringform_rubric'), 'list');
        $layoutgrp[] = $mform->createElement('radio', 'config_layout', '',
                get_string('layoutgrid', 'peeringform_rubric'), 'grid');
        $mform->addGroup($layoutgrp, 'layoutgrp', get_string('layout', 'peeringform_rubric'), array('<br />'), false);
    ...
    protected function validation_inner($data, $files) {
        ...
        for ($i = 0; isset($data['dimensionid__idx_'.$i]); $i++) {
            ...
            if (!$atleastonelevel) {
                $errors['level__idx_'.$i.'__idy_0'] = get_string('mustdefinelevel', 'peeringform_rubric');
            }
        }
