 ...
 * @return bool
 */
function peeringform_rubric_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peeringform_rubric', array('id' => $itemid ,'peeringid' => $peering->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peeringform_rubric/$filearea/$itemid/$relativepath";
 ...
 */
class peering_rubric_strategy implements peering_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        $this->descriptionopts = array_merge(array('context' => $this->peering->context), $this->descriptionopts);
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $this->peering->context, 'peeringform_rubric', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_edit_rubric_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        if ($DB->record_exists('peeringform_rubric_config', array('peeringid' => $this->peering->id))) {
            $DB->set_field('peeringform_rubric_config', 'layout', $layout, array('peeringid' => $this->peering->id));
        } else {
            $record = new stdclass();
            $record->peeringid = $this->peering->id;
            $record->layout     = $layout;
            $DB->insert_record('peeringform_rubric_config', $record, false);
        }
        ...
        foreach ($data as $record) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('peeringform_rubric', $record);
            } else {
                // exiting field
                $DB->update_record('peeringform_rubric', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $this->peering->context, 'peeringform_rubric', 'description', $record->id);
            $DB->update_record('peeringform_rubric', $record);
            ...
            foreach ($record->levels as $level) {
                ...
                if (empty($level->id)) {
                    // new field
                    $level->id = $DB->insert_record('peeringform_rubric_levels', $level);
                } else {
                    // exiting field
                    $DB->update_record('peeringform_rubric_levels', $level);
                }
            }
        }
        $DB->delete_records_list('peeringform_rubric_levels', 'id', $deletelevs);
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $this->peering->context->id, 'peeringform_rubric', 'description',
        ...
        $formclassname = 'peering_rubric_' . $this->config->layout . '_assessment_form';
    ...
    public function get_dimensions_info() {
        ...
        $sql = 'SELECT d.id AS id, MIN(l.grade) AS min, MAX(l.grade) AS max, 1 AS weight
                  FROM {peeringform_rubric} d
            INNER JOIN {peeringform_rubric_levels} l ON (d.id = l.dimensionid)
                 WHERE d.peeringid = :peeringid
              GROUP BY d.id';
    ...
    public static function delete_instance($peeringid) {
        ...
        $dimensions = $DB->get_records('peeringform_rubric', array('peeringid' => $peeringid), '', 'id');
        $DB->delete_records_list('peeringform_rubric_levels', 'dimensionid', array_keys($dimensions));
        $DB->delete_records('peeringform_rubric', array('peeringid' => $peeringid));
        $DB->delete_records('peeringform_rubric_config', array('peeringid' => $peeringid));
    }
    ...
    protected function load_fields() {
        ...
        $sql = "SELECT r.id AS rid, r.sort, r.description, r.descriptionformat,
                  ...
                  FROM {peeringform_rubric} r
             LEFT JOIN {peeringform_rubric_levels} l ON (l.dimensionid = r.id)
                 WHERE r.peeringid = :peeringid
    ...
    protected function load_config() {
        ...
        if (!$config = $DB->get_record('peeringform_rubric_config', array('peeringid' => $this->peering->id), 'layout')) {
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($this->peering->context->id, 'peeringform_rubric', 'description', $id);
            }
        }
        $DB->delete_records_list('peeringform_rubric', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peering_edit_rubric_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        for ($i = 0; $i < $raw->norepeats; $i++) {
            ...
            $cook[$i]->peeringid           = $this->peering->id;
            $cook[$i]->sort                 = $i + 1;
     ...
     * @return array [int dimensionid] => stdclass peering_grades record
     */
