 ...
 * @return bool
 */
function peeringform_comments_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peeringform_comments', array('id' => $itemid ,'peeringid' => $peering->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peeringform_comments/$filearea/$itemid/$relativepath";
 ...
 */
class peering_comments_strategy implements peering_strategy {
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'peeringform_comments', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_edit_comments_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $records    = $data->comments;  // records to be saved into {peeringform_comments}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id         = $DB->insert_record('peeringform_comments', $record);
            } else {
                // exiting field
                $DB->update_record('peeringform_comments', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts,
                                                      $PAGE->context, 'peeringform_comments', 'description', $record->id);
            $DB->update_record('peeringform_comments', $record);
        }
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'peeringform_comments', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_comments_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function save_assessment(stdclass $assessment, stdclass $data) {
        ...
        $this->peering->set_peer_grade($assessment->id, 100.00000);
        return 100.0000;
    }
    ...
    public function get_dimensions_info() {
        ...
        $dimrecords = $DB->get_records('peeringform_comments', array('peeringid' => $this->peering->id), 'sort', 'id');
        $diminfo = array();
    ...
    public static function delete_instance($peeringid) {
        ...
        $DB->delete_records('peeringform_comments', array('peeringid' => $peeringid));
    }
    ...
    protected function load_fields() {
        global $DB;
        return $DB->get_records('peeringform_comments', array('peeringid' => $this->peering->id), 'sort');
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        foreach ($ids as $id) {
            if (!empty($id)) {   // to prevent accidental removal of all files in the area
                $fs->delete_area_files($PAGE->context->id, 'peeringform_comments', 'description', $id);
            }
        }
        $DB->delete_records_list('peeringform_comments', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peering_edit_comments_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->comments = array();        // records to be stored in {peeringform_comments}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->comments[$i]                     = new stdclass();
            $cook->comments[$i]->id                 = $raw->{'dimensionid__idx_'.$i};
            $cook->comments[$i]->peeringid         = $this->peering->id;
