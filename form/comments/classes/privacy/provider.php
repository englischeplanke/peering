...
/**
 * Provides the class {@link peeringform_comments\privacy\provider}
 *
...
class provider implements \core_privacy\local\metadata\null_provider, \mod_peering\privacy\peeringform_provider {
    ...
    public static function export_assessment_form(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        $sql = "SELECT dim.id, dim.description, dim.descriptionformat, wg.peercomment, wg.peercommentformat
                  ...
                  JOIN {peeringform_comments} dim ON dim.peeringid = w.id
        ...
        foreach ($DB->get_records_sql($sql, $params) as $record) {
            ...
            $record->description = $writer->rewrite_pluginfile_urls($subcontext, 'peeringform_comments',
        ...
        if ($hasdata) {
            ...
            foreach ($dimensionids as $dimensionid) {
                $writer->export_area_files($subcontext, 'peeringform_comments', 'description', $dimensionid);
            }
        }
    }
}
