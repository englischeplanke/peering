 ...
 * @uses moodleform
 */
class peering_edit_strategy_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform = $this->_form;
        $this->peering = $this->_customdata['peering'];
        $this->strategy = $this->_customdata['strategy'];

        $mform->addElement('hidden', 'peeringid', $this->peering->id);        // peeringid
        $mform->setType('peeringid', PARAM_INT);

        $mform->addElement('hidden', 'strategy', $this->peering->strategy);    // strategy name
        ...
        $buttonarray[] = $mform->createElement('submit', 'saveandpreview', get_string('saveandpreview', 'peering'));
