 ...
 * @uses moodleform
 */
class peering_edit_numerrors_strategy_form extends peering_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $plugindefaults     = get_config('peeringform_numerrors');
        ...
        for ($i = 0; $i < $norepeats; $i++) {
            $mform->addElement('header', 'dimension'.$i, get_string('dimensionnumber', 'peeringform_numerrors', $i+1));
            $mform->addElement('hidden', 'dimensionid__idx_'.$i);   // the id in peering_forms
            $mform->setType('dimensionid__idx_'.$i, PARAM_INT);
            $mform->addElement('editor', 'description__idx_'.$i.'_editor',
                    get_string('dimensiondescription', 'peeringform_numerrors'), '', $descriptionopts);
            $mform->addElement('text', 'grade0__idx_'.$i, get_string('grade0', 'peeringform_numerrors'), array('size'=>'15'));
            $mform->setDefault('grade0__idx_'.$i, $plugindefaults->grade0);
            $mform->setType('grade0__idx_'.$i, PARAM_TEXT);
            $mform->addElement('text', 'grade1__idx_'.$i, get_string('grade1', 'peeringform_numerrors'), array('size'=>'15'));
            ...
            $mform->addElement('select', 'weight__idx_'.$i,
                    get_string('dimensionweight', 'peeringform_numerrors'), peering::available_dimension_weights_list());
            $mform->setDefault('weight__idx_'.$i, 1);
        }

        $mform->addElement('header', 'mappingheader', get_string('grademapping', 'peeringform_numerrors'));
        $mform->addElement('static', 'mappinginfo', get_string('maperror', 'peeringform_numerrors'),
                                                            get_string('mapgrade', 'peeringform_numerrors'));
        ...
        $mform->addElement('submit', 'noadddims', get_string('addmoredimensions', 'peeringform_numerrors',
                peering_numerrors_strategy::ADDDIMS));
