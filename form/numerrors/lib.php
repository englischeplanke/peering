 ...
 * @return bool
 */
function peeringform_numerrors_pluginfile($course, $cm, $context, $filearea, array $args, $forcedownload, array $options=array()) {
    global $DB;
    ...
    if (!$dimension = $DB->get_record('peeringform_numerrors', array('id' => $itemid ,'peeringid' => $peering->id))) {
        send_file_not_found();
    }
    ...
    $fullpath = "/$context->id/peeringform_numerrors/$filearea/$itemid/$relativepath";
 ...
 */
class peering_numerrors_strategy implements peering_strategy {
    ...
    /** @var peering the parent peering instance */
    protected $peering;
     ...
     * Constructor
     *
     * @param peering $peering The peering instance record
     * @return void
     */
    ...
    public function get_edit_strategy_form($actionurl=null) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            ...
            $fields = file_prepare_standard_editor($fields, 'description__idx_'.$i, $this->descriptionopts,
                $PAGE->context, 'peeringform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_edit_numerrors_strategy_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public function save_edit_strategy_form(stdclass $data) {
        ...
        $peeringid = $data->peeringid;
        $norepeats  = $data->norepeats;

        $data       = $this->prepare_database_fields($data);
        $records    = $data->numerrors; // data to be saved into {peeringform_numerrors}
        $mappings   = $data->mappings;  // data to be saved into {peeringform_numerrors_map}
        ...
        for ($i=0; $i < $norepeats; $i++) {
            ...
            if (empty($record->id)) {
                // new field
                $record->id = $DB->insert_record('peeringform_numerrors', $record);
            } else {
                // exiting field
                $DB->update_record('peeringform_numerrors', $record);
            }
            ...
            $record = file_postupdate_standard_editor($record, 'description', $this->descriptionopts, $PAGE->context,
                                                      'peeringform_numerrors', 'description', $record->id);
            $DB->update_record('peeringform_numerrors', $record);
        }
        ...
        foreach ($data->mappings as $nonegative => $grade) {
            ...
            if (isset($this->mappings[$nonegative])) {
                $DB->set_field('peeringform_numerrors_map', 'grade', $grade,
                            array('peeringid' => $this->peering->id, 'nonegative' => $nonegative));
            } else {
                $DB->insert_record('peeringform_numerrors_map',
                            (object)array('peeringid' => $this->peering->id, 'nonegative' => $nonegative, 'grade' => $grade));
            }
        }
        ...
        $sql = "DELETE FROM {peeringform_numerrors_map}
                      WHERE (($insql nonegative > :maxnonegative) AND (peeringid = :peeringid))";
        $params['maxnonegative'] = $maxnonegative;
        $params['peeringid']   = $this->peering->id;
    ...
    public function get_assessment_form(moodle_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        ...
        for ($i = 0; $i < $nodimensions; $i++) {
            $fields->{'description__idx_'.$i} = file_rewrite_pluginfile_urls($fields->{'description__idx_'.$i},
                'pluginfile.php', $PAGE->context->id, 'peeringform_numerrors', 'description', $fields->{'dimensionid__idx_'.$i});
        }
        ...
        return new peering_numerrors_assessment_form($actionurl, $customdata, 'post', '', $attributes, $editable);
    }
    ...
    public function save_assessment(stdclass $assessment, stdclass $data) {
        ...
        for ($i = 0; $i < $data->nodims; $i++) {
            ...
            if (empty($grade->id)) {
                // new grade
                $grade->id = $DB->insert_record('peering_grades', $grade);
            } else {
                // updated grade
                $DB->update_record('peering_grades', $grade);
            }
        }
    ...
    public function get_assessments_recordset($restrict=null) {
       global $DB;
        ...
        $sql = 'SELECT s.id AS submissionid,
                  ...
                  JOIN {peering_assessments} a ON (a.submissionid = s.id)
                  JOIN {peering_grades} g ON (g.assessmentid = a.id AND g.strategy = :strategy)
                 WHERE s.example=0 AND s.peeringid=:peeringid'; // to be cont.
        $params = array('peeringid' => $this->peering->id, 'strategy' => $this->peering->strategy);
    ...
    public function get_dimensions_info() {
        ...
        $params = array('peeringid' => $this->peering->id);
        $dimrecords = $DB->get_records('peeringform_numerrors', array('peeringid' => $this->peering->id), 'sort', 'id,weight');
    ...
    /**
     * Is a given scale used by the instance of peering?
     *
     * This grading strategy does not use scales.
     *
     * @param int $scaleid id of the scale to check
     * @param int|null $peeringid id of peering instance to check, checks all in case of null
     * @return bool
     */
    public static function scale_used($scaleid, $peeringid=null) {
        return false;
    }
    ...
    public static function delete_instance($peeringid) {
        ...
        $DB->delete_records('peeringform_numerrors', array('peeringid' => $peeringid));
        $DB->delete_records('peeringform_numerrors_map', array('peeringid' => $peeringid));
    }
    ...
    /**
     * Loads the fields of the assessment form currently used in this peering
     *
    ...
    protected function load_fields() {
        ...
        $sql = 'SELECT *
                  FROM {peeringform_numerrors}
                 WHERE peeringid = :peeringid
                 ORDER BY sort';
        $params = array('peeringid' => $this->peering->id);
    ...
    protected function load_mappings() {
        global $DB;
        return $DB->get_records('peeringform_numerrors_map', array('peeringid' => $this->peering->id), 'nonegative',
                                'nonegative,grade'); // we can use nonegative as key here as it must be unique within peering
    }
    ...
    protected function delete_dimensions(array $ids) {
        ...
        $fs         = get_file_storage();
        foreach ($ids as $id) {
            $fs->delete_area_files($PAGE->context->id, 'peeringform_numerrors', 'description', $id);
        }
        $DB->delete_records_list('peeringform_numerrors', 'id', $ids);
    }

    /**
     * Prepares data returned by {@link peering_edit_numerrors_strategy_form} so they can be saved into database
     *
    ...
    protected function prepare_database_fields(stdclass $raw) {
        ...
        $cook->numerrors    = array();          // to be stored in {peeringform_numerrors}
        $cook->mappings     = array();          // to be stored in {peeringform_numerrors_map}

        for ($i = 0; $i < $raw->norepeats; $i++) {
            $cook->numerrors[$i]                        = new stdclass();
            $cook->numerrors[$i]->id                    = $raw->{'dimensionid__idx_'.$i};
            $cook->numerrors[$i]->peeringid            = $this->peering->id;
     ...
     * @return array of filtered records from the table peering_grades
     */
    ...
    protected function update_peer_grade(stdclass $assessment) {
        ...
        if (!is_null($suggested)) {
            $this->peering->set_peer_grade($assessment->id, $suggested);
        }
        return $suggested;
    }
