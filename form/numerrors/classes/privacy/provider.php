...
/**
 * Provides the class {@link peeringform_numerrors\privacy\provider}
 *
...
class provider implements \core_privacy\local\metadata\null_provider, \mod_peering\privacy\peeringform_provider {
    ...
    public static function export_assessment_form(\stdClass $user, \context $context, array $subcontext, int $assessmentid) {
        ...
        $sql = "SELECT dim.id, dim.peeringid, dim.description, dim.descriptionformat, dim.grade0, dim.grade1, dim.weight,
                  ...
                  JOIN {peeringform_numerrors} dim ON dim.peeringid = w.id
             LEFT JOIN {peering_grades} wg ON wg.strategy = :strategy
        ...
        foreach ($DB->get_records_sql($sql, $params) as $record) {
            ...
            $record->description = $writer->rewrite_pluginfile_urls($subcontext, 'peeringform_numerrors',
                'description', $record->id, $record->description);
            $peeringid = $record->peeringid;
            $dimensionids[] = $record->id;
            unset($record->id);
            unset($record->peeringid);
            $data[] = $record;
        }
        ...
        if ($hasdata) {
            ...
            foreach ($dimensionids as $dimensionid) {
                $writer->export_area_files($subcontext, 'peeringform_numerrors', 'description', $dimensionid);
            }
            foreach ($DB->get_records('peeringform_numerrors_map', ['peeringid' => $peeringid], 'nonegative',
                    'id, nonegative, grade') as $mapping) {
                $writer->export_metadata($subcontext, 'map_'.$mapping->nonegative.'_errors', $mapping->grade,
                    get_string('privacy:export:metadata:map', 'peeringform_numerrors', [
