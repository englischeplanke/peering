...
require_once($CFG->dirroot . '/mod/peering/classes/portfolio_caller.php');

/**
 * Unit tests for mod_peering_portfolio_caller class
 *
 * @package    mod_peering
...
class portfolio_caller_test extends \advanced_testcase {
    ...
    protected function setUp(): void {
        ...
        $peering = $this->getDataGenerator()->create_module('peering', ['course' => $course]);
        $this->cm = get_coursemodule_from_instance('peering', $peering->id, $course->id, false, MUST_EXIST);
        $this->peering = new testable_peering($peering, $this->cm, $course);
    }
    ...
    /**
     * Test the method mod_peering_portfolio_caller::load_data()
     */
    public function test_load_data() {
        ...
        $portfoliocaller = new \mod_peering_portfolio_caller(['id' => $this->peering->cm->id, 'submissionid' => $subid1]);
    ...
    /**
     * Test the method mod_peering_portfolio_caller::get_return_url()
     */
    public function test_get_return_url() {
        ...
        $expected = new \moodle_url('/mod/peering/submission.php', ['cmid' => $this->peering->cm->id, 'id' => $subid1]);
        $actual = new \moodle_url($portfoliocaller->get_return_url());
        $this->assertTrue($expected->compare($actual));
    }

    /**
     * Test the method mod_peering_portfolio_caller::get_navigation()
     */
    ...
    /**
     * Test the method mod_peering_portfolio_caller::check_permissions()
     */
    public function test_check_permissions_exportownsubmissionassessment() {
        ...
        $this->getDataGenerator()->enrol_user($student1->id, $this->peering->course->id, $roleids['student']);
        $this->getDataGenerator()->enrol_user($student2->id, $this->peering->course->id, $roleids['student']);
        $peeringgenerator = $this->getDataGenerator()->get_plugin_generator('mod_peering');
        $subid1 = $peeringgenerator->create_submission($this->peering->id, $student1->id);
        $asid1 = $peeringgenerator->create_assessment($subid1, $student2->id);
        ...
        role_change_permission($roleids['student'], $context, 'mod/peering:exportsubmissions', CAP_PREVENT);
        $this->assertFalse($portfoliocaller->check_permissions());

        role_change_permission($roleids['student'], $context, 'mod/peering:exportsubmissions', CAP_ALLOW);
        $this->assertTrue($portfoliocaller->check_permissions());
    }

    /**
     * Test the method mod_peering_portfolio_caller::get_sha1()
     */
     ...
     * Assert that this function can return the name of the module ('peering').
     */
    public function test_display_name() {
        $this->resetAfterTest(true);

        $name = \mod_peering_portfolio_caller::display_name();
        $this->assertEquals(get_string('pluginname', 'mod_peering'), $name);
    }
}
