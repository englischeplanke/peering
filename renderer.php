...
/**
 * peering module renderering methods are defined here
 *
 * @package    mod_peering
...
/**
 * peering module renderer class
 *
 * @copyright 2024 Johann Mellin <johann.mellin@tuhh.de>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_peering_renderer extends plugin_renderer_base {

    ////////////////////////////////////////////////////////////////////////////
    // External API - methods to render peering renderable components
    ////////////////////////////////////////////////////////////////////////////

    /**
     * Renders the tertiary nav for the allocation pages
     *
     * @param \mod_peering\output\actionbar $actionbar
     * @return bool|string the rendered output
     */
    public function render_allocation_menu(\mod_peering\output\actionbar $actionbar): string {
        return $this->render_from_template('mod_peering/action_bar', $actionbar->export_for_template($this));
    }

    /**
     * Renders peering message
     *
     * @param peering_message $message to display
     * @return string html code
     */
    protected function render_peering_message(peering_message $message) {
        ...
        case peering_message::TYPE_OK:
            $sty = 'ok';
            break;
        case peering_message::TYPE_ERROR:
            $sty = 'error';
            break;
        default:
            $sty = 'info';
    ...
    /**
     * Renders full peering submission
     *
     * @param peering_submission $submission
     * @return string HTML
     */
    protected function render_peering_submission(peering_submission $submission) {
        global $CFG;
        ...
        if (!$anonymous) {
            ...
            $a->url             = $userurl->out();
            $byfullname         = get_string('byfullname', 'peering', $a);
        ...
        $created = get_string('userdatecreated', 'peering', userdate($submission->timecreated));
        $o .= $this->output->container($created, 'userdate created');

        if ($submission->timemodified > $submission->timecreated) {
            $modified = get_string('userdatemodified', 'peering', userdate($submission->timemodified));
        ...
        $content = file_rewrite_pluginfile_urls($submission->content, 'pluginfile.php', $this->page->context->id,
                                                        'mod_peering', 'submission_content', $submission->id);
     ...
     *
     * @param peering_submission_summary $summary
     * @return string text to be echo'ed
     */
    protected function render_peering_submission_summary(peering_submission_summary $summary) {
        ...
        if ($summary->status == 'notgraded') {
            $classes    .= ' notgraded';
            $gradestatus = $this->output->container(get_string('nogradeyet', 'peering'), 'grade-status');

        } else if ($summary->status == 'graded') {
            $classes    .= ' graded';
            $gradestatus = $this->output->container(get_string('alreadygraded', 'peering'), 'grade-status');
        }
        ...
        $created = get_string('userdatecreated', 'peering', userdate($summary->timecreated));
        $o .= $this->output->container($created, 'userdate created');

        if ($summary->timemodified > $summary->timecreated) {
            $modified = get_string('userdatemodified', 'peering', userdate($summary->timemodified));
            $o .= $this->output->container($modified, 'userdate modified');
        }
    ...
    /**
     * Renders full peering example submission
     *
     * @param peering_example_submission $example
     * @return string HTML
     */
    protected function render_peering_example_submission(peering_example_submission $example) {
        ...
        $content = file_rewrite_pluginfile_urls($example->content, 'pluginfile.php', $this->page->context->id,
                                                        'mod_peering', 'submission_content', $example->id);
     ...
     *
     * @param peering_example_submission_summary $summary
     * @return string text to be echo'ed
     */
    protected function render_peering_example_submission_summary(peering_example_submission_summary $summary) {
        ...
        // additional info
        if ($summary->status == 'notgraded') {
            $o .= $this->output->container(get_string('nogradeyet', 'peering'), 'example-info nograde');
        } else {
            $o .= $this->output->container(get_string('gradeinfo', 'peering' , $summary->gradeinfo), 'example-info grade');
        }
     ...
     *
     * @param peering_user_plan $plan prepared for the user
     * @return string html code to be displayed
     */
    protected function render_peering_user_plan(peering_user_plan $plan) {
        $o  = '';    // Output HTML code.
        $numberofphases = count($plan->phases);
        $o .= html_writer::start_tag('div', array(
            'class' => 'userplan',
            'aria-labelledby' => 'mod_peering-userplanheading',
            'aria-describedby' => 'mod_peering-userplanaccessibilitytitle',
        ));
        $o .= html_writer::span(get_string('userplanaccessibilitytitle', 'peering', $numberofphases),
            'accesshide', array('id' => 'mod_peering-userplanaccessibilitytitle'));
        $o .= html_writer::link('#mod_peering-userplancurrenttasks', get_string('userplanaccessibilityskip', 'peering'),
        ...
        foreach ($plan->phases as $phasecode => $phase) {
            ...
            if ($phase->active) {
                ...
                $actions .= get_string('userplancurrentphase', 'peering').' '.$icon;
                ...
                foreach ($phase->actions as $action) {
                    if ($action->type === 'switchphase') {
                        if ($phasecode == peering::PHASE_ASSESSMENT && $plan->peering->phase == peering::PHASE_SUBMISSION
                                && $plan->peering->phaseswitchassessment) {
                            $icon = new pix_icon('i/scheduled', get_string('switchphaseauto', 'mod_peering'));
                        } else {
                            $icon = new pix_icon('i/marker', get_string('switchphase'.$phasecode, 'mod_peering'));
                        }
            ...
            if ($phase->active) {
                $title = html_writer::span($phase->title, 'phasetitle', ['id' => 'mod_peering-userplancurrenttasks']);
                $classes .= ' active';
            } else {
     ...
     *
     * @param peering_allocation_result $result as returned by the allocator's init() method
     * @return string HTML to be echoed
     */
    protected function render_peering_allocation_result(peering_allocation_result $result) {
        global $CFG;

        $status = $result->get_status();

        if (is_null($status) or $status == peering_allocation_result::STATUS_VOID) {
            debugging('Attempt to render peering_allocation_result with empty status', DEBUG_DEVELOPER);
            return '';
        }

        switch ($status) {
        case peering_allocation_result::STATUS_FAILED:
            if ($message = $result->get_message()) {
                $message = new peering_message($message, peering_message::TYPE_ERROR);
            } else {
                $message = new peering_message(get_string('allocationerror', 'peering'), peering_message::TYPE_ERROR);
            }
            break;

        case peering_allocation_result::STATUS_CONFIGURED:
            if ($message = $result->get_message()) {
                $message = new peering_message($message, peering_message::TYPE_INFO);
            } else {
                $message = new peering_message(get_string('allocationconfigured', 'peering'), peering_message::TYPE_INFO);
            }
            break;

        case peering_allocation_result::STATUS_EXECUTED:
            if ($message = $result->get_message()) {
                $message = new peering_message($message, peering_message::TYPE_OK);
            } else {
                $message = new peering_message(get_string('allocationdone', 'peering'), peering_message::TYPE_OK);
            }
            break;
    ...
    /**
     * Renders the peering grading report
     *
     * @param peering_grading_report $gradingreport
     * @return string html code
     */
    protected function render_peering_grading_report(peering_grading_report $gradingreport) {
        ...
        $sortbysubmisstiontitle = $this->helper_sortable_heading(get_string('submission', 'peering'), 'submissiontitle',
                $options->sortby, $options->sorthow);
        $sortbysubmisstionlastmodified = $this->helper_sortable_heading(get_string('submissionlastmodified', 'peering'),
        ...
        if ($options->peeringphase != peering::PHASE_SUBMISSION) {
            $table->head[] = $this->helper_sortable_heading(get_string('receivedgrades', 'peering'));
            if ($options->showsubmissiongrade) {
                $table->head[] = $this->helper_sortable_heading(get_string('submissiongradeof', 'peering', $data->maxgrade),
                        'submissiongrade', $options->sortby, $options->sorthow);
            }
            $table->head[] = $this->helper_sortable_heading(get_string('givengrades', 'peering'));
            if ($options->showgradinggrade) {
                $table->head[] = $this->helper_sortable_heading(get_string('gradinggradeof', 'peering', $data->maxgradinggrade),
        ...
        foreach ($grades as $participant) {
            ...
            if ($numofreceived > 0 and $numofgiven > 0) {
                $numoftrs       = peering::lcm($numofreceived, $numofgiven);
            ...
            for ($tr = 0; $tr < $numoftrs; $tr++) {
                ...
                if ($tr % $spanreceived == 0) {
                    ...
                    $cell->text = $this->helper_grading_report_assessment($assessment, $options->showreviewernames, $userinfo,
                            get_string('gradereceivedfrom', 'peering'));
                ...
                if ($tr % $spangiven == 0) {
                    ...
                    $cell->text = $this->helper_grading_report_assessment($assessment, $options->showauthornames, $userinfo,
                            get_string('gradegivento', 'peering'));
     ...
     *
     * @param peering_feedback_author $feedback
     * @return string HTML
     */
    protected function render_peering_feedback_author(peering_feedback_author $feedback) {
     ...
     *
     * @param peering_feedback_reviewer $feedback
     * @return string HTML
     */
    protected function render_peering_feedback_reviewer(peering_feedback_reviewer $feedback) {
        return $this->helper_render_feedback($feedback);
    }
     ...
     *
     * @param peering_feedback_author|peering_feedback_reviewer $feedback
     * @return string HTML
     */
    private function helper_render_feedback($feedback) {
        ...
        $o .= $this->output->container_start('header');
        $o .= $this->output->heading(get_string('feedbackby', 'peering', s(fullname($feedback->get_provider()))), 3, 'title');
     ...
     *
     * @param peering_assessment $assessment
     * @return string HTML
     */
    protected function render_peering_assessment(peering_assessment $assessment) {
        ...
        if (!empty($assessment->title)) {
            ...
            $title = get_string('assessment', 'peering');
        }
        ...
        if (!$anonymous) {
            ...
            $a->url     = $userurl->out();
            $byfullname = get_string('assessmentby', 'peering', $a);
            $oo         = $this->output->container($userpic, 'picture');
            $oo        .= $this->output->container($byfullname, 'fullname');
        ...
        if (is_null($assessment->realgrade)) {
            $o .= $this->output->container(
                get_string('notassessed', 'peering'),
                'grade nograde'
            );
        } else {
            $a              = new stdClass();
            ...
            $o .= $this->output->container(
                get_string('gradeinfo', 'peering', $a),
                'grade'
            );

            if (!is_null($assessment->weight) and $assessment->weight != 1) {
                $o .= $this->output->container(
                    get_string('weightinfo', 'peering', $assessment->weight),
                    'weight'
                );
            }
        }
        ...
        if (!is_null($assessment->form)) {
            $o .= print_collapsible_region_start('assessment-form-wrapper', uniqid('peering-assessment'),
                    get_string('assessmentform', 'peering'), 'peering-viewlet-assessmentform-collapsed', false, true);
     ...
     *
     * @param peering_example_assessment $assessment
     * @return string HTML
     */
    protected function render_peering_example_assessment(peering_example_assessment $assessment) {
        return $this->render_peering_assessment($assessment);
    }

    /**
     * Renders the reference assessment of an example submission
     *
     * @param peering_example_reference_assessment $assessment
     * @return string HTML
     */
    protected function render_peering_example_reference_assessment(peering_example_reference_assessment $assessment) {
        return $this->render_peering_assessment($assessment);
    }
     ...
     * @return string HTML
     */
    protected function overall_feedback(peering_assessment $assessment) {
        ...
        $o = print_collapsible_region($o, 'overall-feedback-wrapper', uniqid('peering-overall-feedback'),
                get_string('overallfeedback', 'peering'), 'peering-viewlet-overallfeedback-collapsed', false, true);

        return $o;
    }

    /**
     * Renders a perpage selector for peering listings
     *
    ...
    public function perpage_selector($current=10) {
        ...
        $select = new single_select($this->page->url, 'perpage', $options, '', array('' => get_string('showingperpagechange', 'mod_peering')));
        $select->label = get_string('showingperpage', 'mod_peering', $current);
        $select->method = 'post';
    ...
    /**
     * Render the initials bars for peering.
     *
     * @param peering $peering the current peering of initial bars.
     * @param moodle_url $url base URL object.
     * @return string HTML.
     */
    public function initials_bars(peering $peering, moodle_url $url): string {
        $ifirst = $peering->get_initial_first();
        $ilast = $peering->get_initial_last();
     ...
     *
     * @param peering_final_grades $grades with the info about grades in the gradebook
     * @return string HTML
     */
    protected function render_peering_final_grades(peering_final_grades $grades) {
        ...
        if (!empty($grades->submissiongrade)) {
            ...
            $out .= html_writer::tag(
                'div',
                html_writer::tag('div', get_string('submissiongrade', 'mod_peering'), array('class' => 'gradetype')) .
        ...
        if (!empty($grades->assessmentgrade)) {
            ...
            $out .= html_writer::tag(
                'div',
                html_writer::tag('div', get_string('gradinggrade', 'mod_peering'), array('class' => 'gradetype')) .
    ...
    protected function helper_submission_attachments($submissionid, $format = 'html') {
        ...
        $files  = $fs->get_area_files($ctx->id, 'mod_peering', 'submission_attachment', $submissionid);
        ...
        foreach ($files as $file) {
            ...
            $fileurl    = moodle_url::make_pluginfile_url($ctx->id, 'mod_peering', 'submission_attachment',
                            $submissionid, $filepath, $filename, true);
            $embedurl   = moodle_url::make_pluginfile_url($ctx->id, 'mod_peering', 'submission_attachment',
    ...
    protected function helper_user_plan_tasks(array $tasks) {
        ...
        foreach ($tasks as $taskcode => $task) {
            ...
            if ($task->completed === true) {
                $classes .= ' completed';
                $accessibilitytext .= get_string('taskdone', 'peering') . ' ';
            } else if ($task->completed === false) {
                $classes .= ' fail';
                $accessibilitytext .= get_string('taskfail', 'peering') . ' ';
            } else if ($task->completed === 'info') {
                $classes .= ' info';
                $accessibilitytext .= get_string('taskinfo', 'peering') . ' ';
            } else {
                $accessibilitytext .= get_string('tasktodo', 'peering') . ' ';
            }
    ...
    protected function helper_sortable_heading($text, $sortid=null, $sortby=null, $sorthow=null) {
        ...
        if (!is_null($sortid)) {
            if ($sortby !== $sortid or $sorthow !== 'ASC') {
                ...
                $out .= $this->output->action_icon($url, new pix_icon('t/sort_asc', get_string('sortasc', 'peering')),
                    null, array('class' => 'iconsort sort asc'));
            }
            if ($sortby !== $sortid or $sorthow !== 'DESC') {
                $url = new moodle_url($this->page->url);
                $url->params(array('sortby' => $sortid, 'sorthow' => 'DESC'));
                $out .= $this->output->action_icon($url, new pix_icon('t/sort_desc', get_string('sortdesc', 'peering')),
    ...
    protected function helper_grading_report_submission(stdclass $participant) {
        ...
        if (is_null($participant->submissionid)) {
            $out = $this->output->container(get_string('nosubmissionfound', 'peering'), 'info');
        } else {
            $url = new moodle_url('/mod/peering/submission.php',
            ...
            $lastmodified = get_string('userdatemodified', 'peering', userdate($participant->submissionmodified));
    ...
    protected function helper_grading_report_assessment($assessment, $shownames, array $userinfo, $separator) {
        ...
        if (is_null($assessment)) {
            return get_string('nullgrade', 'peering');
        }
        $a = new stdclass();
        $a->grade = is_null($assessment->grade) ? get_string('nullgrade', 'peering') : $assessment->grade;
        $a->gradinggrade = is_null($assessment->gradinggrade) ? get_string('nullgrade', 'peering') : $assessment->gradinggrade;
        ...
        if (is_null($assessment->gradinggradeover)) {
            if ($a->weight == 1) {
                $grade = get_string('formatpeergrade', 'peering', $a);
            } else {
                $grade = get_string('formatpeergradeweighted', 'peering', $a);
            }
        } else {
            $a->gradinggradeover = $assessment->gradinggradeover;
            if ($a->weight == 1) {
                $grade = get_string('formatpeergradeover', 'peering', $a);
            } else {
                $grade = get_string('formatpeergradeoverweighted', 'peering', $a);
            }
        }
        $url = new moodle_url('/mod/peering/assessment.php',
    ...
    protected function helper_grading_report_grade($grade, $over=null) {
        $a = new stdclass();
        $a->grade = is_null($grade) ? get_string('nullgrade', 'peering') : $grade;
        if (is_null($over)) {
            $text = get_string('formataggregatedgrade', 'peering', $a);
        } else {
            $a->over = is_null($over) ? get_string('nullgrade', 'peering') : $over;
            $text = get_string('formataggregatedgradeover', 'peering', $a);
        }
        return $text;
    }
