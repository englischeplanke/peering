...
/**
 * Class for exporting partial peering data.
 *
 * @package    mod_peering
...
/**
 * Class for exporting partial peering data (some fields are only viewable by admins).
 *
 * @copyright  2017 Juan Leyva <juan@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class peering_summary_exporter extends exporter {
    ...
    protected static function define_properties() {
        ...
        return array(
            ...
            'course' => array(
                'type' => PARAM_INT,
                'description' => 'Course id this peering is part of.',
            ),
            'name' => array(
                'type' => PARAM_TEXT,
            ...
            'intro' => array(
                'default' => '',
                'type' => PARAM_RAW,
                'description' => 'peering introduction text.',
                'null' => NULL_ALLOWED,
            ),
            'introformat' => array(
                ...
                'description' => 'peering intro text format.',
            ),
            'lang' => array(
                'type' => PARAM_LANG,
            ...
            'phase' => array(
                'type' => PARAM_INT,
                'default' => 0,
                'description' => 'The current phase of peering (0 = not available, 1 = submission, 2 = assessment, 3 = closed).',
                'optional' => true,
            ),
            'useexamples' => array(
                'type' => PARAM_BOOL,
            ...
            'strategy' => array(
                'type' => PARAM_PLUGIN,
                'description' => 'The type of the current grading strategy used in this peering.',
                'optional' => true,
            ),
            'evaluation' => array(
                'type' => PARAM_PLUGIN,
            ...
            'conclusion' => array(
                'type' => PARAM_RAW,
                'description' => 'A text to be displayed at the end of the peering.',
                'optional' => true,
                'null' => NULL_ALLOWED,
            ),
            'conclusionformat' => array(
                ...
                'type' => PARAM_INT,
                'default' => FORMAT_MOODLE,
                'description' => 'peering conclusion text format.',
            ),
    ...
    protected function get_other_values(renderer_base $output) {
        ...
        $values['introfiles'] = external_util::get_area_files($context->id, 'mod_peering', 'intro', false, false);

        if (!empty($this->data->instructauthors)) {
            $values['instructauthorsfiles'] = external_util::get_area_files($context->id, 'mod_peering', 'instructauthors');
        }

        if (!empty($this->data->instructreviewers)) {
            $values['instructreviewersfiles'] = external_util::get_area_files($context->id, 'mod_peering', 'instructreviewers');
        }

        if (!empty($this->data->conclusion)) {
            $values['conclusionfiles'] = external_util::get_area_files($context->id, 'mod_peering', 'conclusion');
        }
