...
class actionbar implements renderable, templatable {
     ...
     * @param \peering $peering The peering object.
     */
    public function __construct(moodle_url $currenturl, \peering $peering) {
    ...
    public function export_for_template(renderer_base $output): array {
        $allocators = \peering::installed_allocators();
        $menu = [];

        foreach (array_keys($allocators) as $methodid) {
            $selectorname = get_string('pluginname', 'peeringallocation_' . $methodid);
            $menu[$this->peering->allocation_url($methodid)->out(false)] = $selectorname;
        }
