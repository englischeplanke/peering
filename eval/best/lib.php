 ...
 */
class peering_best_evaluation extends peering_evaluation {

    /** @var peering the parent peering instance */
    protected $peering;

    /** @var the recently used settings in this peering */
    protected $settings;

    /**
     * Constructor
     *
     * @param peering $peering The peering api instance
     * @return void
     */
    public function __construct(peering $peering) {
        global $DB;
        $this->peering = $peering;
        $this->settings = $DB->get_record('peeringeval_best_settings', array('peeringid' => $this->peering->id));
    }

    /**
     * Calculates the grades for assessment and updates 'gradinggrade' fields in 'peering_assessments' table
     *
    ...
    public function update_grading_grades(stdclass $settings, $restrict=null) {
        ...
        // Remember the recently used settings for this peering.
        if (empty($this->settings)) {
            $record = new stdclass();
            $record->peeringid = $this->peering->id;
            $record->comparison = $settings->comparison;
            $DB->insert_record('peeringeval_best_settings', $record);
        } elseif ($this->settings->comparison != $settings->comparison) {
            $DB->set_field('peeringeval_best_settings', 'comparison', $settings->comparison,
                    array('peeringid' => $this->peering->id));
        }

        // Get the grading strategy instance.
        $grader = $this->peering->grading_strategy_instance();
     ...
     *
     * @return peering_best_evaluation_settings_form
     */
    public function get_settings_form(moodle_url $actionurl=null) {
        ...
        return new peering_best_evaluation_settings_form($actionurl, $customdata, 'post', '', $attributes);
    }
    ...
    public static function delete_instance($peeringid) {
        ...
        $DB->delete_records('peeringeval_best_settings', array('peeringid' => $peeringid));
    }
    ...
    protected function process_assessments(array $assessments, array $diminfo, stdclass $settings) {
        ...
        if (is_null($average)) {
            foreach ($assessments as $asid => $assessment) {
                if (!is_null($assessment->gradinggrade)) {
                    $DB->set_field('peering_assessments', 'gradinggrade', null, array('id' => $asid));
                }
            }
            return;
        }
        ...
        foreach ($grades as $assessmentid => $grade) {
            if (grade_floats_different($grade, $assessments[$assessmentid]->gradinggrade)) {
                ...
                $DB->update_record('peering_assessments', $record, true);  // bulk operations expected
            }
        }
 ...
 */
class peering_best_evaluation_settings_form extends peering_evaluation_settings_form {
    ...
    protected function definition_sub() {
        ...
        $plugindefaults = get_config('peeringeval_best');
        $current = $this->_customdata['current'];

        $options = array();
        for ($i = 9; $i >= 1; $i = $i-2) {
            $options[$i] = get_string('comparisonlevel' . $i, 'peeringeval_best');
        }
        $mform->addElement('select', 'comparison', get_string('comparison', 'peeringeval_best'), $options);
        $mform->addHelpButton('comparison', 'comparison', 'peeringeval_best');
