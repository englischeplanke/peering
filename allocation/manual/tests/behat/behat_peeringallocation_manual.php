...
/**
 * Steps definitions related to peeringallocation_manual.
 *
 * @package    peeringallocation_manual
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_peeringallocation_manual extends behat_base {
    /**
     * Manually adds a reviewer for peering participant.
     *
     * This step should start on manual allocation page.
     *
     * @When /^I add a reviewer "(?P<reviewer_name_string>(?:[^"]|\\")*)" for peering participant "(?P<participant_name_string>(?:[^"]|\\")*)"$/
     * @param string $reviewername
     * @param string $participantname
     */
    public function i_add_a_reviewer_for_peering_participant($reviewername, $participantname) {
        ...
        try {
            ...
            $this->find_button(get_string('showallparticipants', 'peeringallocation_manual'))->press();
            $selectnode = $this->find('xpath', $xpathselect);
        }
        ...
        $allocatedtext = behat_context_helper::escape(get_string('allocationadded', 'peeringallocation_manual'));
        $this->find('xpath', "//*[contains(.,$allocatedtext)]");
    }

    /**
     * Manually allocates multiple reviewers in peering.
     *
     * @When /^I allocate submissions in peering "(?P<peering_name_string>(?:[^"]|\\")*)" as:$/
     * @When /^I allocate submissions in peering "(?P<peering_name_string>(?:[^"]|\\")*)" as:"$/
     ...
     */
    public function i_allocate_submissions_in_peering_as($peeringname, TableNode $table) {
        $this->execute("behat_navigation::go_to_breadcrumb_location", $peeringname);
        $this->execute('behat_navigation::i_navigate_to_in_current_page_administration',
            get_string('submissionsallocation', 'peering'));
        $rows = $table->getRows();
        ...
        for ($i = 1; $i < count($rows); $i++) {
            $this->execute(
                'behat_peeringallocation_manual::i_add_a_reviewer_for_peering_participant',
                [
                    $rows[$i][$reviewer],
                    $rows[$i][$participant],
                ]
