 ...
 */
class peering_manual_allocator implements peering_allocator {
    ...
    public function init() {
        ...
        if ($perpage and $perpage > 0 and $perpage <= 1000) {
            require_sesskey();
            set_user_preference('peeringallocation_manual_perpage', $perpage);
            redirect($PAGE->url);
        }
        ...
        case 'new':
            ...
            $submission = $this->peering->get_submission_by_author($authorid);
            if (!$submission) {
                ...
                $res = $this->peering->add_allocation($submission, $reviewerid);
        ...
        case 'del':
            ...
            $assessment     = $this->peering->get_assessment_by_id($assessmentid);
            if ($assessment) {
                if (!$confirmed) {
                    ...
                    if($this->peering->delete_assessment($assessment->id)) {
                        $m[] = self::MSG_DELETED;
                        $m[] = $assessment->authorid;
                        $m[] = $assessment->reviewerid;
                    } else {
    ...
    public function ui() {
        ...
        $output     = $PAGE->get_renderer('peeringallocation_manual');

        $page       = optional_param('page', 0, PARAM_INT);
        $perpage    = get_user_preferences('peeringallocation_manual_perpage', 10);
        $groupid    = groups_get_activity_group($this->peering->cm, true);
        ...
        if ($m) {
            ...
            case self::MSG_ADDED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new peering_message(get_string('allocationadded', 'peeringallocation_manual'),
                    peering_message::TYPE_OK);
                break;
            case self::MSG_EXISTS:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new peering_message(get_string('allocationexists', 'peeringallocation_manual'),
                    peering_message::TYPE_INFO);
                break;
            case self::MSG_NOSUBMISSION:
                $hlauthorid     = $m[1];
                $message        = new peering_message(get_string('nosubmissionfound', 'peering'),
                    peering_message::TYPE_ERROR);
                break;
            case self::MSG_CONFIRM_DEL:
                $hlauthorid     = $m[2];
                $hlreviewerid   = $m[3];
                if ($m[4] == 0) {
                    $message    = new peering_message(get_string('areyousuretodeallocate', 'peeringallocation_manual'),
                        peering_message::TYPE_INFO);
                } else {
                    $message    = new peering_message(get_string('areyousuretodeallocategraded', 'peeringallocation_manual'),
                        peering_message::TYPE_ERROR);
                }
                $url = new moodle_url($PAGE->url, array('mode' => 'del', 'what' => $m[1], 'confirm' => 1, 'sesskey' => sesskey()));
                $label = get_string('iamsure', 'peering');
                $message->set_action($url, $label);
                break;
            case self::MSG_DELETED:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new peering_message(get_string('assessmentdeleted', 'peering'),
                    peering_message::TYPE_OK);
                break;
            case self::MSG_DELETE_ERROR:
                $hlauthorid     = $m[1];
                $hlreviewerid   = $m[2];
                $message        = new peering_message(get_string('assessmentnotdeleted', 'peering'),
                    peering_message::TYPE_ERROR);
                break;
            }
        }

        // fetch the list of ids of all peering participants
        $numofparticipants = $this->peering->count_participants(false, $groupid);
        $participants = $this->peering->get_participants(false, $groupid, $perpage * $page, $perpage);
        ...
        if ($hlauthorid > 0 and $hlreviewerid > 0) {
            ...
            $button = $output->single_button($PAGE->url, get_string('showallparticipants', 'peeringallocation_manual'), 'get');
        } else {
            $button = '';
        }
        ...
        $submissions = $this->peering->get_submissions(array_keys($participants));
        ...
        if ($participants) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid,
                     ...
                     WHERE u.id $participantids AND s.peeringid = :peeringid AND s.example = 0";
        ...
        $data                   = new peeringallocation_manual_allocations();
        $data->peering         = $this->peering;
        $data->allocations      = $allocations;
        $data->userinfo         = $userinfo;
        $data->authors          = $this->peering->get_potential_authors();
        $data->reviewers        = $this->peering->get_potential_reviewers();
        $data->hlauthorid       = $hlauthorid;
        $data->hlreviewerid     = $hlreviewerid;
        $data->selfassessment   = $this->peering->useselfassessment;

        // prepare the group selector
        $groupselector = $output->container(groups_print_activity_menu($this->peering->cm, $PAGE->url, true), 'groupwidget');
 ...
 *
 * @see peering_manual_allocator::ui()
 */
class peeringallocation_manual_allocations implements renderable {

    /** @var peering module instance */
    public $peering;
