 ...
 */
class peeringallocation_manual_renderer extends mod_peering_renderer  {
     ...
     *
     * @param peeringallocation_manual_allocations $data to be displayed
     * @return string html code
     */
    protected function render_peeringallocation_manual_allocations(peeringallocation_manual_allocations $data) {
        ...
        $selfassessment     = $data->selfassessment;    // bool is the self-assessment allowed in this peering?
        ...
        $table->head        = array(get_string('participantreviewedby', 'peering'),
                                    get_string('participant', 'peering'),
                                    get_string('participantrevierof', 'peering'));
    ...
    /**
     * Returns information about the peering participant
     *
     * @return string HTML code
     */
    protected function helper_participant(stdclass $allocation, array $userinfo) {
        ...
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nosubmissionfound', 'peering'), 'info');
        } else {
            $link = $this->peering->submission_url($allocation->submissionid);
            ...
            if (is_null($allocation->submissiongrade)) {
                $o .= $this->output->container(get_string('nogradeyet', 'peering'), array('grade', 'missing'));
            } else {
                $o .= $this->output->container(get_string('alreadygraded', 'peering'), array('grade', 'missing'));
            }
        }
    ...
    protected function helper_reviewers_of_participant(stdclass $allocation, array $userinfo, array $reviewers, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('nothingtoreview', 'peering'), 'info');
        } else {
            $exclude = array();
            if (! $selfassessment) {
            ...
            if ($options) {
                ...
                $select = new single_select($handler, 'by', $options, '', array(''=>get_string('chooseuser', 'peering')), 'addreviewof' . $allocation->userid);
                $select->set_label(get_string('addreviewer', 'peeringallocation_manual'));
    ...
    protected function helper_reviewees_of_participant(stdclass $allocation, array $userinfo, array $authors, $selfassessment) {
        $o = '';
        if (is_null($allocation->submissionid)) {
            $o .= $this->output->container(get_string('withoutsubmission', 'peering'), 'info');
        }
        $exclude = array();
        if (! $selfassessment) {
            $exclude[$allocation->userid] = true;
            $o .= $this->output->container(get_string('selfassessmentdisabled', 'peering'), 'info');
        }
        // todo add an option to exclude users without own submission
        ...
        if ($options) {
            ...
            $select = new single_select($handler, 'of', $options, '', array(''=>get_string('chooseuser', 'peering')), 'addreviewby' . $allocation->userid);
            $select->set_label(get_string('addreviewee', 'peeringallocation_manual'));
            $o .= $this->output->render($select);
        } else {
            $o .= $this->output->container(get_string('nothingtoreview', 'peering'), 'info');
        }
