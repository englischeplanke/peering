 ...
 *
 * @package    peeringallocation_random
 ...
 * Allocates the submissions randomly
 */
class peering_random_allocator implements peering_allocator {
    ...
    public function init() {
        ...
        $this->mform = new peering_random_allocator_form($PAGE->url, $customdata);
     ...
     *
     * @param peering_random_allocator_setting $setting
     * @param peering_allocation_result allocation result logger
     */
    public function execute(peering_random_allocator_setting $settings, peering_allocation_result $result) {

        $authors        = $this->peering->get_potential_authors();
        $authors        = $this->peering->get_grouped($authors);
        $reviewers      = $this->peering->get_potential_reviewers(!$settings->assesswosubmission);
        $reviewers      = $this->peering->get_grouped($reviewers);
        $assessments    = $this->peering->get_all_assessments();
        ...
        if ($settings->numofreviews) {
            ...
            $result->log(get_string('numofrandomlyallocatedsubmissions', 'peeringallocation_random', count($randomallocations)));
        ...
        if ($settings->addselfassessment) {
            ...
            $result->log(get_string('numofselfallocatedsubmissions', 'peeringallocation_random', count($selfallocations)));
            unset($selfallocations);
        }
        if (empty($newallocations)) {
            $result->log(get_string('noallocationtoadd', 'peeringallocation_random'), 'info');
        } else {
            ...
            foreach ($newallocations as $newallocation) {
                ...
                if (isset($allreviewers[$reviewerid])) {
                    ...
                    if (!$allreviewersreloaded) {
                        $allreviewers = $this->peering->get_potential_reviewers(false);
                ...
                if (in_array($newallocation, $newnonexistingallocations)) {
                    $result->log(get_string('allocationaddeddetail', 'peeringallocation_random', $a), 'ok', 1);
                } else {
                    $result->log(get_string('allocationreuseddetail', 'peeringallocation_random', $a), 'ok', 1);
                }
            }
        }
        if ($settings->removecurrent) {
            ...
            // by reviewer
            $result->log(get_string('numofdeallocatedassessment', 'peeringallocation_random', count($delassessments)), 'info');
            foreach ($delassessments as $delassessmentkey => $delassessmentid) {
                ...
                if (!is_null($assessments[$delassessmentid]->grade)) {
                    $result->log(get_string('allocationdeallocategraded', 'peeringallocation_random', $a), 'error', 1);
                    unset($delassessments[$delassessmentkey]);
                } else {
                    $result->log(get_string('assessmentdeleteddetail', 'peeringallocation_random', $a), 'info', 1);
                }
            }
            $this->peering->delete_assessment($delassessments);
        }
        $result->set_status(peering_allocation_result::STATUS_EXECUTED);
    }
    ...
    public function ui() {
        ...
        $message = new peering_message();
        if ($m == self::MSG_SUCCESS) {
            $message->set_text(get_string('randomallocationdone', 'peeringallocation_random'));
            $message->set_type(peering_message::TYPE_OK);
        }
        ...
        $gmode = groups_get_activity_groupmode($this->peering->cm, $this->peering->course);
        if (VISIBLEGROUPS == $gmode or SEPARATEGROUPS == $gmode) {
            $users = $this->peering->get_potential_authors() + $this->peering->get_potential_reviewers();
            $users = $this->peering->get_grouped($users);
            if (isset($users[0])) {
                ...
                if (!empty($nogroupusers)) {
                    ...
                    $a = implode(', ', $list);
                    $out .= $output->box(get_string('nogroupusers', 'peeringallocation_random', $a), 'generalbox warning nogroupusers');
                }
            }
        }

        // TODO $out .= $output->heading(get_string('stats', 'peeringallocation_random'));
     ...
     *
     * @param array $authors grouped of {@see peering::get_potential_authors()}
     * @param array $reviewers grouped by {@see peering::get_potential_reviewers()}
     * @param array $assessments as returned by {@see peering::get_all_assessments()}
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        $submissions    = $this->peering->get_submissions($authorids);
        $submissions    = $this->index_submissions_by_authors($submissions);
        foreach ($newallocations as $newallocation) {
            $reviewerid = key($newallocation);
            $authorid = current($newallocation);
            if (!isset($submissions[$authorid])) {
                throw new moodle_exception('unabletoallocateauthorwithoutsubmission', 'peering');
            }
            $submission = $submissions[$authorid];
            $status = $this->peering->add_allocation($submission, $reviewerid, 1, true);   // todo configurable weight?
            if (peering::ALLOCATION_EXISTS == $status) {
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new moodle_exception('moresubmissionsbyauthor', 'peering');
                }
     ...
     * @param peering_allocation_result $result allocation result logger
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (peering_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            // circles are authors, squares are reviewers
            $result->log(get_string('resultnumperauthor', 'peeringallocation_random', $numofreviews), 'info');
            $allcircles = $authors;
            $allsquares = $reviewers;
            // get current workload
            list($circlelinks, $squarelinks) = $this->convert_assessments_to_links($assessments);
        } elseif (peering_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            $result->log(get_string('resultnumperreviewer', 'peeringallocation_random', $numofreviews), 'info');
            $allcircles = $reviewers;
            $allsquares = $authors;
            // get current workload
            list($squarelinks, $circlelinks) = $this->convert_assessments_to_links($assessments);
        } else {
            throw new moodle_exception('unknownusertypepassed', 'peering');
        }
                    ...
                    while ($keeptrying && (count($circlelinks[$circleid]) < $requiredreviews)) {
                        ...
                        if (NOGROUPS == $gmode) {
                            if (in_array(0, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeers', 'peeringallocation_random'), 'error', 1);
                                break;
                            }
                            $targetgroup = 0;
                            ...
                            if (in_array($circlegroupid, $failedgroups)) {
                                $keeptrying = false;
                                $result->log(get_string('resultnomorepeersingroup', 'peeringallocation_random'), 'error', 1);
                                break;
                            }
                        ...
                        if ($targetgroup === false) {
                            $keeptrying = false;
                            $result->log(get_string('resultnotenoughpeers', 'peeringallocation_random'), 'error', 1);
                            break;
                        }
        ...
        if (peering_random_allocator_setting::NUMPER_REVIEWER == $numper) {
            // circles are reviewers, squares are authors
            foreach ($circlelinks as $circleid => $squares) {
                foreach ($squares as $squareid) {
     ...
     *
     * @param array $assessments array of assessments as returned by {@link peering::get_all_assessments()}
 ...
 */
class peering_random_allocator_setting {
     ...
     * @return peering_random_allocator_setting
     */
