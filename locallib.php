...
/**
 * Library of internal classes and functions for module peering
 *
 * All the peering specific functions, needed to implement the module
 * logic, should go to here. Instead of having bunch of function named
 * peering_something() taking the peering instance as the first
 * parameter, we use a class peering that provides all methods.
 *
 * @package    mod_peering
...
/**
 * Full-featured peering API
 *
 * This wraps the peering database record with a set of methods that are called
 * from the module itself. The class should be initialized right after you get
 * $peering, $cm and $course records at the begining of the script.
 */
class peering {
    ...
    /** the internal code of the peering phases as are stored in the database */
    ...
    /** @var stdclass peering record from database */
    public $dbrecord;
    ...
    /** @var int peering instance identifier */
    public $id;

    /** @var string peering activity name */
    public $name;
    ...
    /** @var int current phase of peering, for example {@link peering::PHASE_SETUP} */
    public $phase;

    /** @var bool optional feature: students practise evaluating on example submissions from teacher */
    public $useexamples;

    /** @var bool optional feature: set autonomous groups in order to let the peers grade each other*/
    public $autonomousgroups;
    ...
    /** @var string type of the current grading strategy used in this peering, for example 'accumulative' */
    public $strategy;
    ...
    /** @var int mode of example submissions support, for example {@link peering::EXAMPLES_VOLUNTARY} */
    public $examplesmode;
    ...
    /**
     * @var peering_strategy grading strategy instance
     * Do not use directly, get the instance using {@link peering::grading_strategy_instance()}
     */
    protected $strategyinstance = null;

    /**
     * @var peering_evaluation grading evaluation instance
     * Do not use directly, get the instance using {@link peering::grading_evaluation_instance()}
     */
    ...
    /**
     * Initializes the peering API instance using the data from DB
     *
     ...
     *
     * @param stdClass $dbrecord peering instance data from {peering} table
     * @param stdClass|cm_info $cm Course module record
     * @param stdClass $course Course record from {course} table
     * @param stdClass $context The context of the peering instance
     */
    public function __construct(stdclass $dbrecord, $cm, $course, stdclass $context=null) {
        ...
        foreach ($this->dbrecord as $field => $value) {
            if (property_exists('peering', $field)) {
                $this->{$field} = $value;
            }
        }
    ...
    public static function installed_allocators() {
        $installed = core_component::get_plugin_list('peeringallocation');
        $forms = array();
        foreach ($installed as $allocation => $allocationpath) {
            if (file_exists($allocationpath . '/lib.php')) {
                $forms[$allocation] = get_string('pluginname', 'peeringallocation_' . $allocation);
            }
        }
    ...
    public static function available_example_modes_list() {
        $options = array();
        $options[self::EXAMPLES_VOLUNTARY]         = get_string('examplesvoluntary', 'peering');
        $options[self::EXAMPLES_BEFORE_SUBMISSION] = get_string('examplesbeforesubmission', 'peering');
        $options[self::EXAMPLES_BEFORE_ASSESSMENT] = get_string('examplesbeforeassessment', 'peering');
        return $options;
    }
    ...
    public static function available_strategies_list() {
        $installed = core_component::get_plugin_list('peeringform');
        $forms = array();
        foreach ($installed as $strategy => $strategypath) {
            if (file_exists($strategypath . '/lib.php')) {
                $forms[$strategy] = get_string('pluginname', 'peeringform_' . $strategy);
            }
        }
        return $forms;
    }
    ...
    public static function available_evaluators_list() {
        $evals = array();
        foreach (core_component::get_plugin_list_with_file('peeringeval', 'lib.php', false) as $eval => $evalpath) {
            $evals[$eval] = get_string('pluginname', 'peeringeval_' . $eval);
        }
        return $evals;
    }
     ...
     * peering 1.x and I just decided to use the same number as in the maximum weight of
    ...
    public static function timestamp_formats($timestamp) {
        ...
        if ($day == $today) {
            $a->distanceday = get_string('daystoday', 'peering');
        } elseif ($day == $yesterday) {
            $a->distanceday = get_string('daysyesterday', 'peering');
        } elseif ($day < $today) {
            $a->distanceday = get_string('daysago', 'peering', $distance);
        } elseif ($day == $tomorrow) {
            $a->distanceday = get_string('daystomorrow', 'peering');
        } elseif ($day > $today) {
            $a->distanceday = get_string('daysleft', 'peering', $distance);
        }
        return $a;
    }
    ...
    public static function normalize_file_extensions($extensions) {
        ...
        debugging('The method peering::normalize_file_extensions() is deprecated.
    ...
    public static function clean_file_extensions($extensions) {
        ...
        debugging('The method peering::clean_file_extensions() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    public static function invalid_file_extensions($extensions, $allowlist) {
        ...
        debugging('The method peering::invalid_file_extensions() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    /**
     * Is the file have allowed to be uploaded to the peering?
     *
    ...
    public static function is_allowed_file_type($filename, $allowlist) {
        ...
        debugging('The method peering::is_allowed_file_type() is deprecated.
            Please use the methods provided by the \core_form\filetypes_util class.', DEBUG_DEVELOPER);
    ...
    /**
     * Fetches all enrolled users with the capability mod/peering:submit in the current peering
     *
    ...
    public function get_potential_authors($musthavesubmission=true, $groupid=0, $limitfrom=0, $limitnum=0) {
        ...
        list($sql, $params) = $this->get_users_with_capability_sql('mod/peering:submit', $musthavesubmission, $groupid);
    ...
    /**
     * Fetches all enrolled users with the capability mod/peering:peerassess in the current peering
     *
     * The returned objects contain properties required by user_picture and are ordered by lastname, firstname.
     * Only users with the active enrolment are returned.
     *
    ...
    public function get_potential_reviewers($musthavesubmission=false, $groupid=0, $limitfrom=0, $limitnum=0) {
        ...
        list($sql, $params) = $this->get_users_with_capability_sql('mod/peering:peerassess', $musthavesubmission, $groupid);
    ...
    /**
     * Fetches all enrolled users that are authors or reviewers (or both) in the current peering
     *
     * The returned objects contain properties required by user_picture and are ordered by lastname, firstname.
     * Only users with the active enrolment are returned.
     *
    ...
    /**
     * Checks if the given user is an actively enrolled participant in the peering
     *
    ...
    public function get_grouped($users) {
        ...
        if ($this->cm->groupingid) {
            // Group peering set to specified grouping - only consider groups
    ...
    /**
     * Returns the list of all allocations (i.e. assigned assessments) in the peering
     *
    ...
    public function get_allocations() {
        ...
        $sql = 'SELECT a.id, a.submissionid, a.reviewerid, s.authorid
            ...
            INNER JOIN {peering_submissions} s ON (a.submissionid = s.id)
                 WHERE s.example = 0 AND s.peeringid = :peeringid';
        $params = array('peeringid' => $this->id);
    ...
    public function count_submissions($authorid='all', $groupid=0) {
        ...
        $sql .= " WHERE s.example = 0 AND s.peeringid = :peeringid";
    ...
    /**
     * Returns submissions from this peering
     *
     * Fetches data from {peering_submissions} and adds some useful information from other
    ...
    public function get_submissions($authorid='all', $groupid=0, $limitfrom=0, $limitnum=0) {
        ...
        $params            = array('peeringid' => $this->id);
        $sql = "SELECT s.id, s.peeringid, s.example, s.authorid, s.timecreated, s.timemodified,
    ...
    /**
     * Returns submissions from this peering that are viewable by the current user (except example submissions).
     *
    ...
    public function get_visible_submissions($authorid = 0, $groupid = 0, $limitfrom = 0, $limitnum = 0) {
        ...
        $from = " FROM {peering_submissions} s";
        $params = array('peeringid' => $this->id);
        ...
        $where = " WHERE s.peeringid = :peeringid AND s.example = 0";

        if (!has_capability('mod/peering:viewallsubmissions', $this->context)) {
            // Check published submissions.
            $peeringclosed = $this->phase == self::PHASE_CLOSED;
            $canviewpublished = has_capability('mod/peering:viewpublishedsubmissions', $this->context);
            if ($peeringclosed && $canviewpublished) {
    ...
    public function get_submission_by_id($id) {
        ...
        // we intentionally check the peeringid here, too, so the peering can't touch submissions
        ...
        $sql = "SELECT s.*, $authorfields, $gradeoverbyfields
            ...
            INNER JOIN {user} u ON (s.authorid = u.id)
             LEFT JOIN {user} g ON (s.gradeoverby = g.id)
                 WHERE s.example = 0 AND s.peeringid = :peeringid AND s.id = :id";
        $params = array('peeringid' => $this->id, 'id' => $id);
    ...
    public function get_submission_by_author($authorid) {
        ...
        $params = array('peeringid' => $this->id, 'authorid' => $authorid);
    ...
    public function get_published_submissions($orderby='finalgrade DESC') {
        ...
        $sql = "SELECT s.id, s.authorid, s.timecreated, s.timemodified,
                  ...
                  FROM {peering_submissions} s
            INNER JOIN {user} u ON (s.authorid = u.id)
                 WHERE s.example = 0 AND s.peeringid = :peeringid AND s.published = 1
              ORDER BY $orderby";
        $params = array('peeringid' => $this->id);
        return $DB->get_records_sql($sql, $params);
    }
    ...
    public function get_example_by_id($id) {
        global $DB;
        return $DB->get_record('peering_submissions',
                array('id' => $id, 'peeringid' => $this->id, 'example' => 1), '*', MUST_EXIST);
    }

    /**
     * Returns the list of example submissions in this peering with reference assessments attached
     *
     * @return array of objects or an empty array
     * @see peering::prepare_example_summary()
     */
    public function get_examples_for_manager() {
        ...
        $sql = 'SELECT s.id, s.title,
                  ...
                  FROM {peering_submissions} s
             LEFT JOIN {peering_assessments} a ON (a.submissionid = s.id AND a.weight = 1)
                 WHERE s.example = 1 AND s.peeringid = :peeringid
              ORDER BY s.title';
        return $DB->get_records_sql($sql, array('peeringid' => $this->id));
    }

    /**
     * Returns the list of all example submissions in this peering with the information of assessments done by the given user
     *
    ...
    public function get_examples_for_reviewer($reviewerid) {
        ...
        $sql = 'SELECT s.id, s.title,
                       a.id AS assessmentid, a.grade, a.gradinggrade
                  FROM {peering_submissions} s
             LEFT JOIN {peering_assessments} a ON (a.submissionid = s.id AND a.reviewerid = :reviewerid AND a.weight = 0)
                 WHERE s.example = 1 AND s.peeringid = :peeringid
              ORDER BY s.title';
        return $DB->get_records_sql($sql, array('peeringid' => $this->id, 'reviewerid' => $reviewerid));
    }

    /**
     * Prepares renderable submission component
     *
     * @param stdClass $record required by {@see peering_submission}
     * @param bool $showauthor show the author-related information
     * @return peering_submission
     */
    public function prepare_submission(stdClass $record, $showauthor = false) {

        $submission         = new peering_submission($this, $record, $showauthor);
     ...
     *
     * @param stdClass $record required by {@see peering_submission_summary}
     * @param bool $showauthor show the author-related information
     * @return peering_submission_summary
     */
    public function prepare_submission_summary(stdClass $record, $showauthor = false) {

        $summary        = new peering_submission_summary($this, $record, $showauthor);
     ...
     *
     * @param stdClass $record required by {@see peering_example_submission}
     * @return peering_example_submission
     */
    public function prepare_example_submission(stdClass $record) {

        $example = new peering_example_submission($this, $record);
     ...
     *
     * @param stdClass $example as returned by {@link peering::get_examples_for_manager()} or {@link peering::get_examples_for_reviewer()}
     * @return peering_example_submission_summary to be rendered
     */
    public function prepare_example_summary(stdClass $example) {

        $summary = new peering_example_submission_summary($this, $example);

        if (is_null($example->grade)) {
            $summary->status = 'notgraded';
            $summary->assesslabel = get_string('assess', 'peering');
        } else {
            $summary->status = 'graded';
            $summary->assesslabel = get_string('reassess', 'peering');
        }
     ...
     * @param peering_assessment_form|null $form as returned by {@link peering_strategy::get_assessment_form()}
     * @param array $options
     * @return peering_assessment
     */
    public function prepare_assessment(stdClass $record, $form, array $options = array()) {

        $assessment             = new peering_assessment($this, $record, $options);
        $assessment->url        = $this->assess_url($record->id);
        $assessment->maxgrade   = $this->real_grade(100);

        if (!empty($options['showform']) and !($form instanceof peering_assessment_form)) {
            debugging('Not a valid instance of peering_assessment_form supplied', DEBUG_DEVELOPER);
        }
     ...
     * @param peering_assessment_form|null $form as returned by {@link peering_strategy::get_assessment_form()}
     * @param array $options
     * @return peering_example_assessment
     */
    public function prepare_example_assessment(stdClass $record, $form = null, array $options = array()) {

        $assessment             = new peering_example_assessment($this, $record, $options);
     ...
     * @param peering_assessment_form|null $form as returned by {@link peering_strategy::get_assessment_form()}
     * @param array $options
     * @return peering_example_reference_assessment
     */
    public function prepare_example_reference_assessment(stdClass $record, $form = null, array $options = array()) {

        $assessment             = new peering_example_reference_assessment($this, $record, $options);
        $assessment->maxgrade   = $this->real_grade(100);
    ...
    public function delete_submission(stdclass $submission) {
        ...
        $assessments = $DB->get_records('peering_assessments', array('submissionid' => $submission->id), '', 'id');
        $this->delete_assessment(array_keys($assessments));

        $fs = get_file_storage();
        $fs->delete_area_files($this->context->id, 'mod_peering', 'submission_content', $submission->id);
        $fs->delete_area_files($this->context->id, 'mod_peering', 'submission_attachment', $submission->id);

        $DB->delete_records('peering_submissions', array('id' => $submission->id));
        ...
        $event = \mod_peering\event\submission_deleted::create($params);
        $event->add_record_snapshot('peering', $this->dbrecord);
        $event->trigger();
    }

    /**
     * Returns the list of all assessments in the peering with some data added
     *
     * Fetches data from {peering_assessments} and adds some useful information from other
        ...
        $sql = "SELECT a.id, a.submissionid, a.reviewerid, a.timecreated, a.timemodified,
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.peeringid = :peeringid AND s.example = 0
              ORDER BY $sort";
        $params['peeringid'] = $this->id;
    ...
    public function get_assessment_by_id($id) {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $authorfields, $overbyfields
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE a.id = :id AND s.peeringid = :peeringid";
        $params = array('id' => $id, 'peeringid' => $this->id);
    ...
    public function get_assessment_of_submission_by_user($submissionid, $reviewerid) {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $authorfields, $overbyfields
                  FROM {peering_assessments} a
            INNER JOIN {user} reviewer ON (a.reviewerid = reviewer.id)
            INNER JOIN {peering_submissions} s ON (a.submissionid = s.id AND s.example = 0)
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.id = :sid AND reviewer.id = :rid AND s.peeringid = :peeringid";
        $params = array('sid' => $submissionid, 'rid' => $reviewerid, 'peeringid' => $this->id);
    ...
    public function get_assessments_of_submission($submissionid) {
        ...
        $sql = "SELECT a.*, s.title, $reviewerfields, $overbyfields
            ...
            INNER JOIN {peering_submissions} s ON (a.submissionid = s.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.example = 0 AND s.id = :submissionid AND s.peeringid = :peeringid
              ORDER BY $sort";
    ...
    public function get_assessments_by_reviewer($reviewerid) {
        ...
        $sql = "SELECT a.*, $reviewerfields, $authorfields, $overbyfields,
            ...
            INNER JOIN {user} author ON (s.authorid = author.id)
             LEFT JOIN {user} overby ON (a.gradinggradeoverby = overby.id)
                 WHERE s.example = 0 AND reviewer.id = :reviewerid AND s.peeringid = :peeringid";
        $params = array('reviewerid' => $reviewerid, 'peeringid' => $this->id);
    ...
    public function add_allocation(stdclass $submission, $reviewerid, $weight=1, $bulk=false) {
        ...
        if ($DB->record_exists('peering_assessments', array('submissionid' => $submission->id, 'reviewerid' => $reviewerid))) {
     ...
     *
     * Removes associated records from the peering_grades table, too.
     *
    ...
    public function delete_assessment($id) {
        ...
        if (is_array($id)) {
            $DB->delete_records_list('peering_grades', 'assessmentid', $id);
            foreach ($id as $itemid) {
                $fs->delete_area_files($this->context->id, 'mod_peering', 'overallfeedback_content', $itemid);
                $fs->delete_area_files($this->context->id, 'mod_peering', 'overallfeedback_attachment', $itemid);
            }
            $DB->delete_records_list('peering_assessments', 'id', $id);

        } else {
            $DB->delete_records('peering_grades', array('assessmentid' => $id));
            $fs->delete_area_files($this->context->id, 'mod_peering', 'overallfeedback_content', $id);
            $fs->delete_area_files($this->context->id, 'mod_peering', 'overallfeedback_attachment', $id);
            $DB->delete_records('peering_assessments', array('id' => $id));
        }
    ...
    public function grading_strategy_instance() {
        ...
        if (is_null($this->strategyinstance)) {
            ...
            $classname = 'peering_' . $this->strategy . '_strategy';
            $this->strategyinstance = new $classname($this);
            if (!in_array('peering_strategy', class_implements($this->strategyinstance))) {
                throw new coding_exception($classname . ' does not implement peering_strategy interface');
            }
        }
     ...
     *
     * @param string $method the name of the peeringeval subplugin
    ...
    public function set_grading_evaluation_method($method) {
        ...
        if (is_readable($evaluationlib)) {
            ...
            $DB->set_field('peering', 'evaluation', $method, array('id' => $this->id));
            return true;
        }
    ...
    public function grading_evaluation_instance() {
        ...
        if (is_null($this->evaluationinstance)) {
            ...
            $classname = 'peering_' . $this->evaluation . '_evaluation';

            $this->evaluationinstance = new $classname($this);
            if (!in_array('peering_evaluation', class_parents($this->evaluationinstance))) {
                throw new coding_exception($classname . ' does not extend peering_evaluation class');
            }
        }
    ...
    public function allocator_instance($method) {
        ...
        $classname = 'peering_' . $method . '_allocator';
        return new $classname($this);
    }

    /**
     * @return moodle_url of this peering's view page
     */
    public function view_url() {
        global $CFG;
        return new moodle_url('/mod/peering/view.php', array('id' => $this->cm->id));
    }

    /**
     * @return moodle_url of the page for editing this peering's grading form
     */
    public function editform_url() {
        global $CFG;
        return new moodle_url('/mod/peering/editform.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return moodle_url of the page for previewing this peering's grading form
     */
    public function previewform_url() {
        global $CFG;
        return new moodle_url('/mod/peering/editformpreview.php', array('cmid' => $this->cm->id));
    }
    ...
    public function assess_url($assessmentid) {
        ...
        return new moodle_url('/mod/peering/assessment.php', array('asid' => $assessmentid));
    }
    ...
    public function exassess_url($assessmentid) {
        global $CFG;
        $assessmentid = clean_param($assessmentid, PARAM_INT);
        return new moodle_url('/mod/peering/exassessment.php', array('asid' => $assessmentid));
    }
    ...
    public function submission_url($id=null) {
        global $CFG;
        return new moodle_url('/mod/peering/submission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }
    ...
    public function exsubmission_url($id) {
        global $CFG;
        return new moodle_url('/mod/peering/exsubmission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }
    ...
    public function compare_url($sid, array $aids) {
        ...
        $url = new moodle_url('/mod/peering/compare.php', array('cmid' => $this->cm->id, 'sid' => $sid));
        $i = 0;
        foreach ($aids as $aid) {
            $url->param("aid{$i}", $aid);
            $i++;
    ...
    public function excompare_url($sid, $aid) {
        global $CFG;
        return new moodle_url('/mod/peering/excompare.php', array('cmid' => $this->cm->id, 'sid' => $sid, 'aid' => $aid));
    }
    ...
    public function allocation_url($method=null) {
        ...
        return new moodle_url('/mod/peering/allocation.php', $params);
    }
    ...
    public function switchphase_url($phasecode) {
        ...
        return new moodle_url('/mod/peering/switchphase.php', array('cmid' => $this->cm->id, 'phase' => $phasecode));
    }
    ...
    public function publishgrades_url() {
        global $CFG;

        return new moodle_url('/mod/peering/publishgrades.php', array('cmid' => $this->cm->id));
    }
    ...
    public function aggregate_url() {
        global $CFG;
        return new moodle_url('/mod/peering/aggregate.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return moodle_url of this peering's toolbox page
     */
    public function toolbox_url($tool) {
        global $CFG;
        return new moodle_url('/mod/peering/toolbox.php', array('id' => $this->cm->id, 'tool' => $tool));
    }

    /**
     * peering wrapper around {@see add_to_log()}
     * @deprecated since 2.7 Please use the provided event classes for logging actions.
     *
     * @param string $action to be logged
     * @param moodle_url $url absolute url as returned by {@see peering::submission_url()} and friends
    ...
    public function log($action, moodle_url $url = null, $info = null, $return = false) {
        ...
        $args = array($this->course->id, 'peering', $action, $logurl, $info, $this->cm->id);
        if ($return) {
            return $args;
        }
    ...
    public function creating_submission_allowed($userid) {
        ...
        $now = time();
        $ignoredeadlines = has_capability('mod/peering:ignoredeadlines', $this->context, $userid);
    ...
    public function assessing_allowed($userid) {
        ...
        if ($this->phase != self::PHASE_ASSESSMENT) {
            ...
            if ($this->phase != self::PHASE_EVALUATION or !has_capability('mod/peering:overridegrades', $this->context, $userid)) {
                return false;
            }
        }
     ...
     *
     * Returns null if example submissions are not enabled in this peering. Otherwise returns
    ...
    public function publish_grades () {
        ...
        $peering = new stdclass();
        foreach ($this as $property => $value) {
            $peering->{$property} = $value;
        }
        $peering->course     = $this->course->id;
        $peering->cmidnumber = $this->cm->id;
        $peering->modname    = 'peering';
        peering_update_grades($peering); 
        return true;     
    }

    /**
     * Switch to a new peering phase
     *
    ...
    public function switch_phase($newphase) {
        ...
        $DB->set_field('peering', 'phase', $newphase, array('id' => $this->id));
        $this->phase = $newphase;
        $eventdata = array(
            'objectid' => $this->id,
            'context' => $this->context,
            'other' => array(
                'peeringphase' => $this->phase
            )
        );
        $event = \mod_peering\event\phase_switched::create($eventdata);
        $event->trigger();
        return true;
    }
    ...
    public function set_peer_grade($assessmentid, $grade) {
        ...
        $DB->update_record('peering_assessments', $data);
        return $grade;
    }

    /**
     * Prepares data object with all peering grades to be rendered
     *
    ...
    public function prepare_grading_report_data($userid, $groupid, $page, $perpage, $sortby, $sorthow) {
        ...
        $canviewall     = has_capability('mod/peering:viewallassessments', $this->context, $userid);
        ...
        if ($canviewall) {
            ...
            // this is an ordinary peering participant (aka student) - display the report just for him/her
        ...
        if ($numofparticipants > 0) {
            ...
            $params['peeringid1'] = $this->id;
            $params['peeringid2'] = $this->id;
            $sqlsort = array();
            ...
            $sql = "SELECT $picturefields, s.title AS submissiontitle, s.timemodified AS submissionmodified,
                      ...
                      FROM {user} u
                 LEFT JOIN {peering_submissions} s ON (s.authorid = u.id AND s.peeringid = :peeringid1 AND s.example = 0)
                 LEFT JOIN {peering_aggregations} ag ON (ag.userid = u.id AND ag.peeringid = :peeringid2)
        ...
        if ($submissions) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid, a.grade, a.gradinggrade, a.gradinggradeover, a.weight,
                      ...
                      JOIN {peering_submissions} s ON (a.submissionid = s.id AND s.example = 0)
        ...
        if ($participants) {
            ...
            $sql = "SELECT a.id AS assessmentid, a.submissionid, a.grade, a.gradinggrade, a.gradinggradeover, a.reviewerid, a.weight,
                      ...
                      FROM {user} u
                      JOIN {peering_assessments} a ON (a.reviewerid = u.id)
                      JOIN {peering_submissions} s ON (a.submissionid = s.id AND s.example = 0)
                      JOIN {user} e ON (s.authorid = e.id)
                     WHERE u.id $participantids AND s.peeringid = :peeringid
                  ORDER BY a.weight DESC, $sort";
     ...
     * clears the calculated grades in peering_assessments. Therefore reviewers have to re-assess
    ...
    public function clear_assessments() {
        ...
        $DB->set_field_select('peering_assessments', 'grade', null, $sql, $params);
        $DB->set_field_select('peering_assessments', 'gradinggrade', null, $sql, $params);
    }
    ...
    public function clear_submission_grades($restrict=null) {
        ...
        $sql = "peeringid = :peeringid AND example = 0";
        $params = array('peeringid' => $this->id);
        ...
        $DB->set_field_select('peering_submissions', 'grade', null, $sql, $params);
    }
    ...
    public function aggregate_submission_grades($restrict=null) {
        ...
        $sql = 'SELECT s.id AS submissionid, s.grade AS submissiongrade,
             ...
             LEFT JOIN {peering_assessments} a ON (a.submissionid = s.id)
                 WHERE s.example=0 AND s.peeringid=:peeringid'; // to be cont.
        $params = array('peeringid' => $this->id);
    ...
    public function clear_grading_grades($restrict=null) {
        ...
        $DB->set_field_select('peering_aggregations', 'gradinggrade', null, $sql, $params);
    }
    ...
    public function aggregate_grading_grades($restrict=null) {
        ...
        $sql = 'SELECT a.reviewerid, a.gradinggrade, a.gradinggradeover,
                  ...
                  FROM {peering_assessments} a
            INNER JOIN {peering_submissions} s ON (a.submissionid = s.id)
             LEFT JOIN {peering_aggregations} ag ON (ag.userid = a.reviewerid AND ag.peeringid = s.peeringid)
                 WHERE s.example=0 AND s.peeringid=:peeringid'; // to be cont.
        $params = array('peeringid' => $this->id);
     ...
     * @return peering_feedbackreviewer_form
     */
    public function get_feedbackreviewer_form($actionurl, stdclass $assessment, $options=array()) {
        ...
        if (is_null($current->gradinggrade)) {
            $current->gradinggrade = get_string('nullgrade', 'peering');
        }
        ...
        return new peering_feedbackreviewer_form($actionurl,
                array('peering' => $this, 'current' => $current, 'editoropts' => array(), 'options' => $options),
                'post', '', null, $editable);
    }
     ...
     * @return peering_feedbackauthor_form
     */
    public function get_feedbackauthor_form($actionurl, stdclass $submission, $options=array()) {
        ...
        if (is_null($current->grade)) {
            $current->grade = get_string('nullgrade', 'peering');
        }
        if (!isset($options['editable'])) {
            $editable = true;   // by default
        } else {
        ...
        return new peering_feedbackauthor_form($actionurl,
                array('peering' => $this, 'current' => $current, 'editoropts' => array(), 'options' => $options),
                'post', '', null, $editable);
    }
     ...
     *
     * The submission grade is returned for users with the capability mod/peering:submit and the
     * assessment grade is returned for users with the capability mod/peering:peerassess. Unless the
     ...
     * @return peering_final_grades|false
     */
    public function get_gradebook_grades($userid) {
        ...
        $gradebook = grade_get_grades($this->course->id, 'mod', 'peering', $this->id, $userid);

        $grades = new peering_final_grades();

        if (has_capability('mod/peering:submit', $this->context, $userid)) {
        ...
        if (has_capability('mod/peering:peerassess', $this->context, $userid)) {
    ...
    /**
     * Performs the reset of this peering instance.
     *
    ...
    public function reset_userdata(stdClass $data) {
        ...
        $componentstr = get_string('pluginname', 'peering').': '.format_string($this->name);
        $status = array();

        if (!empty($data->reset_peering_assessments) or !empty($data->reset_peering_submissions)) {
            ...
            if ($result === true) {
                $status[] = array(
                    ...
                    'item' => get_string('resetassessments', 'mod_peering'),
                    'error' => false,
                );
            } else {
                $status[] = array(
        ...
        if (!empty($data->reset_peering_submissions)) {
            ...
            if ($result === true) {
                $status[] = array(
                    'component' => $componentstr,
                    'item' => get_string('resetsubmissions', 'mod_peering'),
                    'error' => false,
                );
            } else {
                $status[] = array(
        ...
        if (!empty($data->reset_peering_phase)) {
            // Do not use the {@link peering::switch_phase()} here, we do not
     ...
     * check the peering activity group mode.
     *
     * If the peering is not in a group mode, or if it is in the visible group
     * mode, this method returns true. This is consistent with how the
     * {@link groups_get_activity_allowed_groups()} behaves.
     *
     * If the peering is in a separate group mode, the current user has to
    ...
    public function check_group_membership($otheruserid) {
        ...
        if (groups_get_activity_groupmode($this->cm) != SEPARATEGROUPS) {
            // The peering is not in a group mode, or it is in a visible group mode.
            return true;
    ...
    public function check_examples_assessed_before_submission($userid) {
        ...
        if ($this->useexamples and $this->examplesmode == self::EXAMPLES_BEFORE_SUBMISSION
            and !has_capability('mod/peering:manageexamples', $this->context)) {
    ...
    public function set_module_viewed() {
        ...
        $event = \mod_peering\event\course_module_viewed::create($eventdata);
    ...
    public function validate_submission_data($data) {
        ...
        if (empty($data['id']) and empty($data['example'])) {
            ...
            $sql = "SELECT COUNT(s.id)
                      FROM {peering_submissions} s
                      JOIN {peering} w ON (s.peeringid = w.id)
                      JOIN {course_modules} cm ON (w.id = cm.instance)
                      JOIN {modules} m ON (m.name = 'peering' AND m.id = cm.module)
                     WHERE cm.id = ? AND s.authorid = ? AND s.example = 0";

            if ($DB->count_records_sql($sql, array($data['cmid'], $USER->id))) {
                $errors['title'] = get_string('err_multiplesubmissions', 'mod_peering');
            }
        }
        // Get the peering record by id or cmid, depending on whether we're creating or editing a submission.
        if (empty($data['peeringid'])) {
            $peering = $DB->get_record_select('peering', 'id = (SELECT instance FROM {course_modules} WHERE id = ?)',
                    [$data['cmid']]);
        } else {
            $peering = $DB->get_record('peering', ['id' => $data['peeringid']]);
        }
        ...
        } else if ($peering->submissiontypefile == peering_SUBMISSION_TYPE_DISABLED && !empty($data['attachment_filemanager'])) {
            $errors['attachment_filemanager'] = get_string('submissiontypedisabled', 'mod_peering');
        }

        if ($peering->submissiontypetext == peering_SUBMISSION_TYPE_REQUIRED && html_is_blank($data['content_editor']['text'])) {
            $errors['content_editor'] = get_string('err_required', 'form');
        } else if ($peering->submissiontypetext == peering_SUBMISSION_TYPE_DISABLED && !empty($data['content_editor']['text'])) {
            $errors['content_editor'] = get_string('submissiontypedisabled', 'mod_peering');
        }
        ...
        if ($peering->submissiontypetext != peering_SUBMISSION_TYPE_REQUIRED
                && $peering->submissiontypefile != peering_SUBMISSION_TYPE_REQUIRED
                && empty($attachments) && html_is_blank($data['content_editor']['text'])) {
            $errors['content_editor'] = get_string('submissionrequiredcontent', 'mod_peering');
            $errors['attachment_filemanager'] = get_string('submissionrequiredfile', 'mod_peering');
        }
    ...
    public function edit_submission($submission) {
        ...
        $logdata = null;
        if (is_null($submission->id)) {
            $submission->id = $DB->insert_record('peering_submissions', $submission);
            $params['objectid'] = $submission->id;
            $event = \mod_peering\event\submission_created::create($params);
            $event->trigger();
        } else {
            if (empty($submission->id) or empty($submission->id) or ($submission->id != $submission->id)) {
                throw new moodle_exception('err_submissionid', 'peering');
            }
        }
        $params['objectid'] = $submission->id;

        // Save and relink embedded images and save attachments.
        if ($this->submissiontypetext != peering_SUBMISSION_TYPE_DISABLED) {
            $submission = file_postupdate_standard_editor($submission, 'content', $this->submission_content_options(),
                    $this->context, 'mod_peering', 'submission_content', $submission->id);
        }

        $submission = file_postupdate_standard_filemanager($submission, 'attachment', $this->submission_attachment_options(),
            $this->context, 'mod_peering', 'submission_attachment', $submission->id);
        ...
        $DB->update_record('peering_submissions', $submission);
        $event = \mod_peering\event\submission_updated::create($params);
        $event->add_record_snapshot('peering', $this->dbrecord);
        $event->trigger();

        // Send submitted content for plagiarism detection.
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'mod_peering', 'submission_attachment', $submission->id);

        $params['other']['content'] = $submission->content;
        $params['other']['pathnamehashes'] = array_keys($files);

        $event = \mod_peering\event\assessable_uploaded::create($params);
    ...
    public function check_view_assessment($assessment, $submission) {
        ...
        $canviewallassessments  = has_capability('mod/peering:viewallassessments', $this->context);
        $canviewallsubmissions  = has_capability('mod/peering:viewallsubmissions', $this->context);
        ...
        if ($isauthor and !$isreviewer and !$canviewallassessments and $this->phase != self::PHASE_CLOSED) {
            // Authors can see assessments of their work at the end of peering only.
            throw new \moodle_exception('nopermissions', 'error', $this->view_url(),
                'view assessment of own work before peering is closed');
        }
    }
    ...
    public function check_edit_assessment($assessment, $submission) {
        ...
        if (!$assessed) {
            throw new moodle_exception($notice, 'mod_peering');
        }
    }
    ...
    public function edit_assessment($assessment, $submission, $data, $strategy) {
        ...
        $cansetassessmentweight = has_capability('mod/peering:allocate', $this->context);

        // Let the grading strategy subplugin save its data.
        $rawgrade = $strategy->save_assessment($assessment, $data);

        // Store the data managed by the peering core.
        ...
        if (isset($data->feedbackauthor_editor)) {
            ...
            $coredata = file_postupdate_standard_editor($coredata, 'feedbackauthor', $this->overall_feedback_content_options(),
                $this->context, 'mod_peering', 'overallfeedback_content', $assessment->id);
        ...
        if (isset($data->feedbackauthorattachment_filemanager)) {
            ...
            $coredata = file_postupdate_standard_filemanager($coredata, 'feedbackauthorattachment',
                $this->overall_feedback_attachment_options(), $this->context, 'mod_peering', 'overallfeedback_attachment',
                $assessment->id);
        ...
        if (count((array)$coredata) > 1 ) {
            $DB->update_record('peering_assessments', $coredata);
            $params = array(
                'relateduserid' => $submission->authorid,
            ...
            if (is_null($assessment->grade)) {
                // All peering_assessments are created when allocations are made. The create event is of more use located here.
                $event = \mod_peering\event\submission_assessed::create($params);
                $event->trigger();
            } else {
                $params['other']['grade'] = $assessment->grade;
                $event = \mod_peering\event\submission_reassessed::create($params);
                $event->trigger();
            }
        }
        return $rawgrade;
    ...
    public function evaluate_assessment($assessment, $data, $cansetassessmentweight, $canoverridegrades) {
        ...
        $DB->update_record('peering_assessments', $record);
    }
    ...
    public function set_submission_viewed($submission) {
        ...
        $event = \mod_peering\event\submission_viewed::create($params);
        $event->trigger();
    }
    ...
    public function init_initial_bar(): void {
        ...
        if (empty($SESSION->mod_peering->initialbarprefs['id-'.$this->context->id])) {
            $SESSION->mod_peering = new stdClass();
            $SESSION->mod_peering->initialbarprefs['id-'.$this->context->id] = [];
        }
        if (!empty($SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_first'])) {
            $this->initialbarprefs['i_first'] = $SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_first'];
        }
        if (!empty($SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_last'])) {
            $this->initialbarprefs['i_last'] = $SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_last'];
        }
        if (!is_null($ifirst)) {
            $this->initialbarprefs['i_first'] = $ifirst;
            $SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_first'] = $ifirst;
        }

        if (!is_null($ilast)) {
            $this->initialbarprefs['i_last'] = $ilast;
            $SESSION->mod_peering->initialbarprefs['id-'.$this->context->id]['i_last'] = $ilast;
        }
    }
    ...
    protected function aggregate_grading_grades_process(array $assessments, $timegraded = null) {
        ...
        if (grade_floats_different($finalgrade, $current)) {
            ...
            if (is_null($agid)) {
                ...
                $record = new stdclass();
                $record->peeringid = $this->id;
                $record->userid = $reviewerid;
                $record->gradinggrade = $finalgrade;
                $record->timegraded = $timegraded;
                $record->id = $DB->insert_record('peering_aggregations', $record);
                $params['objectid'] = $record->id;
                $event = \mod_peering\event\assessment_evaluated::create($params);
                $event->trigger();
            } else {
                $record = new stdclass();
                $record->id = $agid;
                $record->gradinggrade = $finalgrade;
                $record->timegraded = $timegraded;
                $DB->update_record('peering_aggregations', $record);
                $params['objectid'] = $agid;
                $event = \mod_peering\event\assessment_reevaluated::create($params);
                $event->trigger();
            }
        }
    }
    ...
    /**
     * Returns SQL to fetch all enrolled users with the given capability in the current peering
     *
    ...
    protected function get_users_with_capability_sql($capability, $musthavesubmission, $groupid) {
        ...
        if ($musthavesubmission) {
            $sql .= " JOIN {peering_submissions} ws ON (ws.authorid = u.id AND ws.example = 0 AND ws.peeringid = :peeringid{$inc}) ";
            $params['peeringid'.$inc] = $this->id;
        }
    ...
    /**
     * Returns SQL statement that can be used to fetch all actively enrolled participants in the peering
     *
     * @param bool $musthavesubmission if true, return only users who have already submitted
     * @param int $groupid 0 means ignore groups, any other value limits the result by group id
     * @return array of (string)sql, (array)params
    ...
    protected function get_participants_sql($musthavesubmission=false, $groupid=0) {
        ...
        list($sql1, $params1) = $this->get_users_with_capability_sql('mod/peering:submit', $musthavesubmission, $groupid);
        list($sql2, $params2) = $this->get_users_with_capability_sql('mod/peering:peerassess', $musthavesubmission, $groupid);
    ...
    /**
     * @return array of available peering phases
     */
    ...
    protected function log_convert_url(moodle_url $fullurl) {
        ...
        if (!isset($baseurl)) {
            $baseurl = new moodle_url('/mod/peering/');
            $baseurl = $baseurl->out();
        }
    ...
    protected function reset_userdata_assessments(stdClass $data) {
        ...
        $sql = "SELECT a.id
                 ...
                 WHERE s.peeringid = :peeringid
                       AND (s.example = 0 OR (s.example = 1 AND a.weight = 0))";

        $assessments = $DB->get_records_sql($sql, array('peeringid' => $this->id));
        $this->delete_assessment(array_keys($assessments));

        $DB->delete_records('peering_aggregations', array('peeringid' => $this->id));
    ...
    /**
     * Hard set the peering phase to the setup one.
     */
    protected function reset_phase() {
        global $DB;

        $DB->set_field('peering', 'phase', self::PHASE_SETUP, array('id' => $this->id));
 ...
 */
class peering_user_plan implements renderable {
    ...
    /**
     * Prepare an individual peering plan for the given user.
     *
     * @param peering $peering instance
     * @param int $userid whom the plan is prepared for
     */
    public function __construct(peering $peering, $userid) {
        global $DB;

        $this->peering = $peering;
        $this->userid   = $userid;

        //echo "boffer".$peering->autonomousgroups;

        //echo "biffer" .$this->get_submissions_to_grade();
        //---------------------------------------------------------
        // * SETUP | submission | assessment | evaluation | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phasesetup', 'peering');
        $phase->tasks = array();
        if (has_capability('moodle/course:manageactivities', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskintro', 'peering');
            $task->link = $peering->updatemod_url();
            $task->completed = !(trim($peering->intro) == '');
            $phase->tasks['intro'] = $task;
        }
        if (has_capability('moodle/course:manageactivities', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskinstructauthors', 'peering');
            $task->link = $peering->updatemod_url();
            $task->completed = !(trim($peering->instructauthors) == '');
            $phase->tasks['instructauthors'] = $task;
        }
        if (has_capability('mod/peering:editdimensions', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('editassessmentform', 'peering');
            $task->link = $peering->editform_url();
            if ($peering->grading_strategy_instance()->form_ready()) {
                $task->completed = true;
            } elseif ($peering->phase > peering::PHASE_SETUP) {
                $task->completed = false;
            }
            $phase->tasks['editform'] = $task;
        }
        if ($peering->useexamples and has_capability('mod/peering:manageexamples', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('prepareexamples', 'peering');
            if ($DB->count_records('peering_submissions', array('example' => 1, 'peeringid' => $peering->id)) > 0) {
                $task->completed = true;
            } elseif ($peering->phase > peering::PHASE_SETUP) {
                $task->completed = false;
            }
        ...
        if (empty($phase->tasks) and $peering->phase == peering::PHASE_SETUP) {
            // if we are in the setup phase and there is no task (typical for students), let us
            // display some explanation what is going on
            $task = new stdclass();
            $task->title = get_string('undersetup', 'peering');
            $task->completed = 'info';
            $phase->tasks['setupinfo'] = $task;
        }
        $this->phases[peering::PHASE_SETUP] = $phase;

        //---------------------------------------------------------
        // setup | * SUBMISSION | assessment | evaluation | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phasesubmission', 'peering');
        $phase->tasks = array();
        if (has_capability('moodle/course:manageactivities', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskinstructreviewers', 'peering');
            $task->link = $peering->updatemod_url();
            if (trim($peering->instructreviewers)) {
                $task->completed = true;
            } elseif ($peering->phase >= peering::PHASE_ASSESSMENT) {
                $task->completed = false;
            }
            $phase->tasks['instructreviewers'] = $task;
        }
        if ($peering->useexamples and $peering->examplesmode == peering::EXAMPLES_BEFORE_SUBMISSION
                and has_capability('mod/peering:submit', $peering->context, $userid, false)
                    and !has_capability('mod/peering:manageexamples', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('exampleassesstask', 'peering');
            ...
            $task->details = get_string('exampleassesstaskdetails', 'peering', $a);
        ...
        if (has_capability('mod/peering:submit', $peering->context, $userid, false)) {
            $task = new stdclass();
            $task->title = get_string('tasksubmit', 'peering');
            $task->link = $peering->submission_url();
            if ($DB->record_exists('peering_submissions', array('peeringid'=>$peering->id, 'example'=>0, 'authorid'=>$userid))) {
                $task->completed = true;
            } elseif ($peering->phase >= peering::PHASE_ASSESSMENT) {
                $task->completed = false;
            } else {
        ...
        if (has_capability('mod/peering:allocate', $peering->context, $userid)) {
            if ($peering->phaseswitchassessment) {
                $task = new stdClass();
                $allocator = $DB->get_record('peeringallocation_scheduled', array('peeringid' => $peering->id));
                if (empty($allocator)) {
                    $task->completed = false;
                ...
                $task->title = get_string('setup', 'peeringallocation_scheduled');
                $task->link = $peering->allocation_url('scheduled');
                $phase->tasks['allocatescheduled'] = $task;
            }
            $task = new stdclass();
            $task->title = get_string('allocate', 'peering');
            $task->link = $peering->allocation_url();
            $numofauthors = $peering->count_potential_authors(false);
            $numofsubmissions = $DB->count_records('peering_submissions', array('peeringid'=>$peering->id, 'example'=>0));
            $sql = 'SELECT COUNT(s.id) AS nonallocated
                      FROM {peering_submissions} s
                 LEFT JOIN {peering_assessments} a ON (a.submissionid=s.id)
                     WHERE s.peeringid = :peeringid AND s.example=0 AND a.submissionid IS NULL';
            $params['peeringid'] = $peering->id;
            $numnonallocated = $DB->count_records_sql($sql, $params);
            if ($numofsubmissions == 0) {
                $task->completed = null;
            } elseif ($numnonallocated == 0) {
                $task->completed = true;
            } elseif ($peering->phase > peering::PHASE_SUBMISSION) {
                $task->completed = false;
            } else {
            ...
            $task->details  = get_string('allocatedetails', 'peering', $a);
            unset($a);
            $phase->tasks['allocate'] = $task;

            if ($numofsubmissions < $numofauthors and $peering->phase >= peering::PHASE_SUBMISSION) {
                $task = new stdclass();
                $task->title = get_string('someuserswosubmission', 'peering');
                $task->completed = 'info';
        ...
        if ($peering->submissionstart) {
            $task = new stdclass();
            $task->title = get_string('submissionstartdatetime', 'peering', peering::timestamp_formats($peering->submissionstart));
            $task->completed = 'info';
            $phase->tasks['submissionstartdatetime'] = $task;
        }
        if ($peering->submissionend) {
            $task = new stdclass();
            $task->title = get_string('submissionenddatetime', 'peering', peering::timestamp_formats($peering->submissionend));
            $task->completed = 'info';
            $phase->tasks['submissionenddatetime'] = $task;
        }
        if (($peering->submissionstart < time()) and $peering->latesubmissions) {
            ...
            if (has_capability('mod/peering:switchphase', $peering->context, $userid) ||
                    (!$peering->get_submission_by_author($userid) && $peering->submissionend < time())) {
                $task = new stdclass();
                $task->title = get_string('latesubmissionsallowed', 'peering');
                $task->completed = 'info';
        ...
        if (isset($phase->tasks['submissionstartdatetime']) or isset($phase->tasks['submissionenddatetime'])) {
            if (has_capability('mod/peering:ignoredeadlines', $peering->context, $userid)) {
                $task = new stdclass();
                $task->title = get_string('deadlinesignored', 'peering');
                $task->completed = 'info';
                $phase->tasks['deadlinesignored'] = $task;
            }
        }
        $this->phases[peering::PHASE_SUBMISSION] = $phase;

        //---------------------------------------------------------
        // setup | submission | * ASSESSMENT | evaluation | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseassessment', 'peering');
        $phase->tasks = array();
        $phase->isreviewer = has_capability('mod/peering:peerassess', $peering->context, $userid);
        if ($peering->phase == peering::PHASE_SUBMISSION and $peering->phaseswitchassessment
                and has_capability('mod/peering:switchphase', $peering->context, $userid)) {
            $task = new stdClass();
            $task->title = get_string('switchphase30auto', 'mod_peering', peering::timestamp_formats($peering->submissionend));
            $task->completed = 'info';
            $phase->tasks['autoswitchinfo'] = $task;
        }
        if ($peering->useexamples and $peering->examplesmode == peering::EXAMPLES_BEFORE_ASSESSMENT
                and $phase->isreviewer and !has_capability('mod/peering:manageexamples', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('exampleassesstask', 'peering');
            $examples = $peering->get_examples_for_reviewer($userid);
            $a = new stdclass();
            $a->expected = count($examples);
            $a->assessed = 0;
            foreach ($examples as $exampleid => $example) {
        ...
        if (empty($phase->tasks['examples']) or !empty($phase->tasks['examples']->completed)) {
            $phase->assessments = $peering->get_assessments_by_reviewer($userid);
            ...
            if ($numofpeers) {
                ...
                $task->title = get_string('taskassesspeers', 'peering');
                $task->details = get_string('taskassesspeersdetails', 'peering', $a);
                unset($a);
                $phase->tasks['assesspeers'] = $task;
            }
            if ($peering->useselfassessment and $numofself) {
                $task = new stdclass();
                if ($numofselftodo == 0) {
                    $task->completed = true;
                } elseif ($peering->phase > peering::PHASE_ASSESSMENT) {
                ...
                $task->title = get_string('taskassessself', 'peering');
                $phase->tasks['assessself'] = $task;
            }
        }
        if ($peering->assessmentstart) {
            $task = new stdclass();
            $task->title = get_string('assessmentstartdatetime', 'peering', peering::timestamp_formats($peering->assessmentstart));
            $task->completed = 'info';
            $phase->tasks['assessmentstartdatetime'] = $task;
        }
        if ($peering->assessmentend) {
            $task = new stdclass();
            $task->title = get_string('assessmentenddatetime', 'peering', peering::timestamp_formats($peering->assessmentend));
            $task->completed = 'info';
        ...
        $this->phases[peering::PHASE_ASSESSMENT] = $phase;

        //---------------------------------------------------------
        // setup | submission | assessment | * EVALUATION | closed
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseevaluation', 'peering');
        $phase->tasks = array();
        if (has_capability('mod/peering:overridegrades', $peering->context)) {
            $expected = $peering->count_potential_authors(false);
            $calculated = $DB->count_records_select('peering_submissions',
                    'peeringid = ? AND (grade IS NOT NULL OR gradeover IS NOT NULL)', array($peering->id));
            $task = new stdclass();
            $task->title = get_string('calculatesubmissiongrades', 'peering');
            $a = new stdclass();
            $a->expected    = $expected;
            $a->calculated  = $calculated;
            $task->details  = get_string('calculatesubmissiongradesdetails', 'peering', $a);
            if ($calculated >= $expected) {
                $task->completed = true;
            } elseif ($peering->phase > peering::PHASE_EVALUATION) {
                $task->completed = false;
            }
            $phase->tasks['calculatesubmissiongrade'] = $task;

            $expected = $peering->count_potential_reviewers(false);
            $calculated = $DB->count_records_select('peering_aggregations',
                    'peeringid = ? AND gradinggrade IS NOT NULL', array($peering->id));
            $task = new stdclass();
            $task->title = get_string('calculategradinggrades', 'peering');
            $a = new stdclass();
            $a->expected    = $expected;
            $a->calculated  = $calculated;
            $task->details  = get_string('calculategradinggradesdetails', 'peering', $a);
            if ($calculated >= $expected) {
                $task->completed = true;
            } elseif ($peering->phase > peering::PHASE_EVALUATION) {
                $task->completed = false;
            ...
            $task = new stdclass();
            $task->title = get_string('evaluategradeswait', 'peering');
            $task->completed = 'info';
        ...
        if (has_capability('moodle/course:manageactivities', $peering->context, $userid)) {
            $task = new stdclass();
            $task->title = get_string('taskconclusion', 'peering');
            $task->link = $peering->updatemod_url();
        ...
        $this->phases[peering::PHASE_EVALUATION] = $phase;

        //---------------------------------------------------------
        // setup | submission | assessment | evaluation | * CLOSED
        //---------------------------------------------------------
        $phase = new stdclass();
        $phase->title = get_string('phaseclosed', 'peering');
        $phase->tasks = array();
        $this->phases[peering::PHASE_CLOSED] = $phase;
        $openedbyusers = array();
...
/*
                if (in_array(30, $openedbyusers )) {
                    echo "Irix enthalten";
                }else{
                    echo "Irix nicht enthalten";                 
                }
        ...
        if (has_capability('mod/peering:switchphase', $peering->context, $userid)) {
            $nextphases = array(
                peering::PHASE_SETUP => peering::PHASE_SUBMISSION,
                peering::PHASE_SUBMISSION => peering::PHASE_ASSESSMENT,
                peering::PHASE_ASSESSMENT => peering::PHASE_EVALUATION,
                peering::PHASE_EVALUATION => peering::PHASE_CLOSED,
            );
            foreach ($this->phases as $phasecode => $phase) {
                if ($phase->active) {
                    if (isset($nextphases[$peering->phase])) {
                        $task = new stdClass();
                        $task->title = get_string('switchphasenext', 'mod_peering');
                        $task->link = $peering->switchphase_url($nextphases[$peering->phase]);
                        $task->details = '';
                        $task->completed = null;
                    ...
                    $action->url  = $peering->switchphase_url($phasecode);
    ...
    /**
     * Returns a list of non-graded submissions of that peering  
     *
     * 
     *
     * @return string
     */
    public function get_submissions_to_grade() {
        global $DB, $USER;
/*
        $sql = 'SELECT a.id, a.submissionid, a.reviewerid, s.authorid
                  FROM {peering_assessments} a
...
*/
     ...
     *
     * @see peering::get_examples_for_reviewer() for the format of returned value
     * @return array
     */
    public function get_examples() {
        if (is_null($this->examples)) {
            $this->examples = $this->peering->get_examples_for_reviewer($this->userid);
        }
 ...
 * peering_submissions table (as returned by {@see peering::get_submission_by_id()}
 * for example) into renderable objects.
 */
abstract class peering_submission_base {

    /** @var bool is the submission anonymous (i.e. contains author information) */
    protected $anonymous;

    /* @var array of columns from peering_submissions that are assigned as properties */
     ...
     *
     * @param peering $peering
     * @param stdClass $submission full record
     * @param bool $showauthor show the author-related information
     * @param array $options additional properties
     */
    public function __construct(peering $peering, stdClass $submission, $showauthor = false) {
 ...
 *
 * @see peering_renderer::render_peering_submission_summary
 */
class peering_submission_summary extends peering_submission_base implements renderable {
 ...
 *
 * @see peering_renderer::render_peering_submission()
 */
class peering_submission extends peering_submission_summary implements renderable {
 ...
 *
 * @see peering::prepare_example_summary()
 * @see peering_renderer::render_peering_example_submission_summary()
 */
class peering_example_submission_summary extends peering_submission_base implements renderable {
 ...
 *
 * @see peering_renderer::render_peering_example_submission()
 */
class peering_example_submission extends peering_example_submission_summary implements renderable {
 ...
 * peering_assessments table (as returned by {@see peering::get_assessment_by_id()}
 * for example) into renderable objects.
 */
abstract class peering_assessment_base {

    /** @var string the optional title of the assessment */
    public $title = '';

    /** @var peering_assessment_form $form as returned by {@link peering_strategy::get_assessment_form()} */
    public $form;
     ...
     * @param array $options additional properties
     */
    public function __construct(peering $peering, stdClass $record, array $options = array()) {
 ...
 */
class peering_assessment extends peering_assessment_base implements renderable {
    ...
    public function get_overall_feedback_content() {
        ...
        if ($this->peering->overallfeedbackmode == 0) {
            return false;
        }
        ...
        $content = file_rewrite_pluginfile_urls($this->feedbackauthor, 'pluginfile.php', $this->peering->context->id,
            'mod_peering', 'overallfeedback_content', $this->id);
        $content = format_text($content, $this->feedbackauthorformat,
            array('overflowdiv' => true, 'context' => $this->peering->context));
    ...
    public function get_overall_feedback_attachments() {
        ...
        if ($this->peering->overallfeedbackfiles == 0) {
            return false;
        }
        ...
        $attachments = array();
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->peering->context->id, 'mod_peering', 'overallfeedback_attachment', $this->id);
        foreach ($files as $file) {
            ...
            $fileurl = moodle_url::make_pluginfile_url($this->peering->context->id, 'mod_peering',
                'overallfeedback_attachment', $this->id, $filepath, $filename, true);
            $previewurl = new moodle_url(moodle_url::make_pluginfile_url($this->peering->context->id, 'mod_peering',
 ...
 */
class peering_example_assessment extends peering_assessment implements renderable {
 ...
 */
class peering_example_reference_assessment extends peering_assessment implements renderable {
 ...
 *
 * @see peering::renderer::render_peering_message()
 */
class peering_message implements renderable {
 ...
 */
class peering_grading_report implements renderable {

    /** @var stdClass returned by {@see peering::prepare_grading_report_data()} */
    protected $data;
     ...
     * (in which later case, the [mod_peering,nullgrade] string shall be displayed)
     *
     * @param stdClass $data prepared by {@link peering::prepare_grading_report_data()}
    ...
    public function export_data_for_external() {
        ...
        foreach ($data->grades as $reportdata) {
            ...
            if ($options->peeringphase == peering::PHASE_SUBMISSION) {
 ...
 */
abstract class peering_feedback {
 ...
 */
class peering_feedback_author extends peering_feedback implements renderable {
 ...
 */
class peering_feedback_reviewer extends peering_feedback implements renderable {
 ...
 */
class peering_final_grades implements renderable {
