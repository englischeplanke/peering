...
$PAGE->set_url($peering->aggregate_url(), compact('confirm', 'page', 'sortby', 'sorthow'));

require_login($course, false, $cm);
require_capability('mod/peering:overridegrades', $PAGE->context);
...
if ($settingsdata = $settingsform->get_data()) {
    $peering->aggregate_submission_grades();           // updates 'grade' in {peering_submissions}
    $evaluator->update_grading_grades($settingsdata);   // updates 'gradinggrade' in {peering_assessments}
    $peering->aggregate_grading_grades();              // updates 'gradinggrade' in {peering_aggregations}
}

redirect(new moodle_url($peering->view_url(), compact('page', 'sortby', 'sorthow')));
